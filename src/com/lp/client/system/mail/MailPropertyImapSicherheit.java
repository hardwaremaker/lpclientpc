...
public class MailPropertyImapSicherheit implements IMailPropertyCtrl {
	...
	private boolean isStarttls(Map<MailPropertyEnum, MailPropertyDto> store) {
		MailPropertyDto dto = store.get(MailPropertyEnum.MailImapStarttlsEnable);
	...
	private boolean isSsl(Map<MailPropertyEnum, MailPropertyDto> store) {
		MailPropertyDto dto = store.get(MailPropertyEnum.MailImapSslEnable);
		return dto != null && Boolean.TRUE.equals(Boolean.valueOf(dto.getCWert()));
	}
	...
	private void setNoneProperties(Map<MailPropertyEnum, String> store) {
		enableImapStarttls(store, false);
		enableImapSsl(store, false);
		enableImapSocketFactoryClass(store, false);
	}

	private void setStarttlsProperties(Map<MailPropertyEnum, String> store) {
		enableImapStarttls(store, true);
	...
	private void setSslProperties(Map<MailPropertyEnum, String> store) {
		enableImapSsl(store, true);
		enableImapSocketFactoryClass(store, true);

		enableImapStarttls(store, false);
	}
	
	private void enableImapSsl(Map<MailPropertyEnum, String> store, boolean enabled) {
		store.put(MailPropertyEnum.MailImapSslEnable, enabled ? "true" : null);
	}
	
	private void enableImapStarttls(Map<MailPropertyEnum, String> store, boolean enabled) {
		store.put(MailPropertyEnum.MailImapStarttlsEnable, enabled ? "true" : null);
	}
	
	private void enableImapSocketFactoryClass(Map<MailPropertyEnum, String> store, boolean enabled) {
		store.put(MailPropertyEnum.MailImapSocketFactoryClass, enabled ? "javax.net.ssl.SSLSocketFactory" : null);
	}
	...
	public void setDataUI(MailPropertiesUI data) {
		data.setImapSicherheitType(securityTypeHolder().get());
	}

	@Override
	public void getDataUI(MailPropertiesUI data) {
		securityTypeHolder().set(data.getImapSicherheitType());
	}
