...
public class PanelVersandauftrag extends PanelBasis {
	...
	private int iAspect;
	private int componentTextRow;
	private Component componentText;
	private WrapperHtmlField wefHtmlText;

	public PanelVersandauftrag(InternalFrame internalFrame, String add2TitleI, Object pk,
			TabbedPaneVersandauftrag tpVersandauftrag, int iAspect) throws Throwable {
	...
	protected void dto2Components() throws Throwable {
		if (versandauftragDto != null) {
			try {
				versandauftragDto = DelegateFactory.getInstance().getJCRDocDelegate()
			...
			if (isHtml()) {
				wefHtmlText.setText(versandauftragDto.getCText());
			} else {
				wefText.setText(versandauftragDto.getCText());
			}
			wtfEmpfaenger.setText(versandauftragDto.getCEmpfaenger());
			wtsfSendezeitpunktWunsch.setTimestamp(versandauftragDto.getTSendezeitpunktwunsch());
			wlaStatus.setText(versandauftragDto.getCStatustext());
			wcbEmpfangsbestaetigung.setSelected(Helper.short2Boolean(versandauftragDto.getBEmpfangsbestaetigung()));
			...
			for (int i = 0; i < versandanhangDto.length; i++) {
				try {
					versandanhangDto[i] = DelegateFactory.getInstance().getJCRDocDelegate()
				...
				if (oinhalt == null || oinhalt.length == 0) {
					myLogger.warn("Der Versandanhang mit der IId " + versandanhangDto[i].getIId()
				...
				File tempFile = null;
				if (posOfDot >= 0) {
					tempFile = File.createTempFile(sPrefix, fileName.substring(posOfDot));
				} else {
					tempFile = File.createTempFile(sPrefix, "");
				}
	...
	public void eventYouAreSelected(boolean bNeedNoYouAreSelectedI) throws Throwable {
		...
		if (key == null || key.equals(LPMain.getLockMeForNew())) {
			...
			versandanhangDto = DelegateFactory.getInstance().getVersandDelegate()
					.VersandanhangFindByVersandauftragIID(versandauftragDto.getIId());
			dto2Components();
		}
		switchTextPanels(isHtml());
	}
	...
	protected void components2Dto() throws Throwable {
		...
		if (isHtml()) {
			versandauftragDto.setCText(wefHtmlText.getHtmlText());
		} else {
			versandauftragDto.setCText(wefText.getText());
		}
		versandauftragDto.setTSendezeitpunktwunsch(wtsfSendezeitpunktWunsch.getTimestamp());
		versandauftragDto.setBEmpfangsbestaetigung(Helper.boolean2Short(wcbEmpfangsbestaetigung.isSelected()));
	}
		...
		if (allMandatoryFieldsSetDlg()) {
			...
			if (bNeuesEmail) {
				for (int i = 0; i < versandanhangDto.length; i++) {
					...
					versandanhangDto[i].setVersandauftragIId(versandauftragDto.getIId());
					versandanhangDto[i] = DelegateFactory.getInstance().getVersandDelegate()
			...
			if (getInternalFrame().getKeyWasForLockMe() == null) {
				getInternalFrame().setKeyWasForLockMe(versandauftragDto.getIId().toString());
			}
	...
	protected void eventActionDelete(ActionEvent e, boolean bAdministrateLockKeyI, boolean bNeedNoDeleteI)
			throws Throwable {
		boolean bDarfAndereVeraendern = DelegateFactory.getInstance().getTheJudgeDelegate()
				.hatRecht(RechteFac.RECHT_LP_ALLE_VERSANDAUFTRAEGE_UD);

		if (!LPMain.getTheClient().getIDPersonal().equals(versandauftragDto.getPersonalIId())
				&& bDarfAndereVeraendern == false) {
			DialogFactory.showModalDialog(LPMain.getTextRespectUISPr("lp.error"),
					LPMain.getTextRespectUISPr("system.error.versandauftrag.bearbeiten"));
			return;
		} else {
			if (iAspect == I_ASPECT_FEHLGESCHLAGEN || iAspect == I_ASPECT_PAPIERKORB) {

				Object[] ids = null;
				if (iAspect == I_ASPECT_FEHLGESCHLAGEN) {
					ids = tpVersandauftrag.getPanelQueryFehlgeschlagen(true).getSelectedIds();
				} else {
					ids = tpVersandauftrag.getPanelQueryPapierkorb(true).getSelectedIds();
				}
				for (int i = 0; i < ids.length; i++) {
					DelegateFactory.getInstance().getVersandDelegate().removeVersandauftrag((Integer) ids[i]);
				}
			} else {
				...
				if (iAspect == I_ASPECT_POSTAUSGANG) {
					...
					versandauftragDto = DelegateFactory.getInstance().getVersandDelegate()
							.versandauftragFindByPrimaryKey(versandauftragDto.getIId());
				...
				DelegateFactory.getInstance().getVersandDelegate().storniereVersandauftrag(versandauftragDto.getIId());
			}
	...
	protected void eventActionUpdate(ActionEvent aE, boolean bNeedNoUpdateI) throws Throwable {
		...
		if (!LPMain.getTheClient().getIDPersonal().equals(versandauftragDto.getPersonalIId())
			...
			if (isHtml()) {
// SP7979 Versandauftrag kann vollstaendiges HTML enthalten, 
// das wollen wir nicht editieren				
//				wefHtmlText.startEditing();
			}
		}
	...
	private void jbInit() throws Throwable {
		...
		wrapperAttachmentViewer = new WrapperAttachmentViewer(getInternalFrame(), "");
		// wrapperPdfField.getLabelDatei().setVisible(false);
		// wrapperPdfField.getButtonDatei().setVisible(false);

		wlaEmpfaenger = new WrapperLabel(LPMain.getTextRespectUISPr("label.empfaenger"));
		...
		wlaBetreff = new WrapperLabel(LPMain.getTextRespectUISPr("label.betreff"));
		...
		wlaSendezeitpunktwunsch = new WrapperLabel(LPMain.getTextRespectUISPr("label.sendezeitpunkt"));
		 ...
		 * .getControlHeight())); wlaEmpfaenger.setPreferredSize(new Dimension(100,
		 * Defaults .getInstance().getControlHeight()));
		 * wtsfSendezeitpunktWunsch.setMinimumSize(new
		 * Dimension(Defaults.sizeFactor(220), Defaults
		 * .getInstance().getControlHeight()));
		 * wtsfSendezeitpunktWunsch.setPreferredSize(new
		 * Dimension(Defaults.sizeFactor(220), Defaults
		 * .getInstance().getControlHeight()));
		 */
		HelperClient.setMinimumAndPreferredSize(wlaEmpfaenger, HelperClient.getSizeFactoredDimension(100));
		HelperClient.setMinimumAndPreferredSize(wtsfSendezeitpunktWunsch, HelperClient.getSizeFactoredDimension(210));

		wcbEmpfangsbestaetigung = new WrapperCheckBox(LPMain.getTextRespectUISPr("lp.empfangsbestaetigung"));
		wefHtmlText = new WrapperHtmlField(getInternalFrame(), LPMain.getTextRespectUISPr("lp.bemerkung"), true);
		...
		this.add(jPanelWorkingOn, new GridBagConstraints(0, 1, 6, 1, 1.0, 1.0, GridBagConstraints.SOUTHEAST,
				GridBagConstraints.BOTH, new Insets(0, 0, 0, 0), 0, 0));
		jPanelWorkingOn.add(wlaEmpfaenger, new GridBagConstraints(0, iZeile, 1, 1, 0.0, 0.0, GridBagConstraints.CENTER,
				GridBagConstraints.HORIZONTAL, new Insets(2, 2, 2, 2), 0, 0));
		jPanelWorkingOn.add(wtfEmpfaenger, new GridBagConstraints(1, iZeile, 5, 1, 1.0, 0.0, GridBagConstraints.CENTER,
				GridBagConstraints.HORIZONTAL, new Insets(2, 2, 2, 2), 0, 0));
		iZeile++;
		jPanelWorkingOn.add(wlaCCEmpfaenger, new GridBagConstraints(0, iZeile, 1, 1, 0.0, 0.0,
				GridBagConstraints.CENTER, GridBagConstraints.HORIZONTAL, new Insets(2, 2, 2, 2), 0, 0));
		jPanelWorkingOn.add(wtfCCEmpfaenger, new GridBagConstraints(1, iZeile, 5, 1, 0.0, 0.0,
				GridBagConstraints.CENTER, GridBagConstraints.HORIZONTAL, new Insets(2, 2, 2, 2), 0, 0));
		iZeile++;
		jPanelWorkingOn.add(wlaBetreff, new GridBagConstraints(0, iZeile, 1, 1, 0.0, 0.0, GridBagConstraints.CENTER,
				GridBagConstraints.HORIZONTAL, new Insets(2, 2, 2, 2), 0, 0));
		jPanelWorkingOn.add(wtfBetreff, new GridBagConstraints(1, iZeile, 5, 1, 0.0, 0.0, GridBagConstraints.CENTER,
				GridBagConstraints.HORIZONTAL, new Insets(2, 2, 2, 2), 0, 0));
		iZeile++;
		jPanelWorkingOn.add(wlaAbsender, new GridBagConstraints(0, iZeile, 1, 1, 0.0, 0.0, GridBagConstraints.CENTER,
				GridBagConstraints.HORIZONTAL, new Insets(2, 2, 2, 2), 0, 0));
		jPanelWorkingOn.add(wtfAbsender, new GridBagConstraints(1, iZeile, 5, 1, 0.0, 0.0, GridBagConstraints.CENTER,
				GridBagConstraints.HORIZONTAL, new Insets(2, 2, 2, 2), 0, 0));
		iZeile++;
		jPanelWorkingOn.add(wlaSendezeitpunktwunsch, new GridBagConstraints(0, iZeile, 1, 1, 0.0, 0.0,
				GridBagConstraints.CENTER, GridBagConstraints.HORIZONTAL, new Insets(2, 2, 2, 2), 0, 0));
		jPanelWorkingOn.add(wtsfSendezeitpunktWunsch, new GridBagConstraints(1, iZeile, 1, 1, 0.0, 0.0,
				GridBagConstraints.CENTER, GridBagConstraints.HORIZONTAL, new Insets(2, 2, 2, 2), 0, 0));
		jPanelWorkingOn.add(wcbEmpfangsbestaetigung, new GridBagConstraints(2, iZeile, 1, 1, 1.0, 0.0,
				GridBagConstraints.CENTER, GridBagConstraints.HORIZONTAL, new Insets(2, 2, 2, 2), 0, 0));

		jPanelWorkingOn.add(wrapperAttachmentViewer, new GridBagConstraints(5, iZeile, 1, 1, 0.0, 0.0,
				GridBagConstraints.CENTER, GridBagConstraints.BOTH, new Insets(0, 0, 0, 0), 0, 0));

		iZeile++;
		jPanelWorkingOn.add(wlaText, new GridBagConstraints(0, iZeile, 1, 1, 0.0, 0.0, GridBagConstraints.CENTER,
				GridBagConstraints.HORIZONTAL, new Insets(2, 2, 2, 2), 0, 0));
...
//				new Insets(2, 2, 2, 2), 0, 0));
		componentTextRow = iZeile;

		iZeile++;
		jPanelWorkingOn.add(new WrapperLabel(LPMain.getTextRespectUISPr("lp.status") + ":"),
				new GridBagConstraints(0, iZeile, 1, 1, 0.0, 0.0, GridBagConstraints.CENTER,
						GridBagConstraints.HORIZONTAL, new Insets(2, 2, 2, 2), 0, 0));
		jPanelWorkingOn.add(wlaStatus, new GridBagConstraints(1, iZeile, 5, 1, 0.0, 0.0, GridBagConstraints.CENTER,
				GridBagConstraints.HORIZONTAL, new Insets(2, 2, 2, 2), 0, 0));
		...
		case I_ASPECT_POSTAUSGANG: {
			createAndSaveAndShowButton("/com/lp/client/res/check2.png",
					LPMain.getTextRespectUISPr("lp.sofortversenden"), ACTION_SPECIAL_SOFORT_VERSENDEN,
		...
		case I_ASPECT_GESENDET: {
			aWhichButtonIUse = new String[] { ACTION_SAVE, ACTION_DISCARD, ACTION_PRINT };
		}
			break;
	...
	protected void eventActionSpecial(ActionEvent e) throws Throwable {
		if (e.getActionCommand().equals(ACTION_SPECIAL_ERNEUT_VERSENDEN)) {
			DelegateFactory.getInstance().getVersandDelegate().sendeVersandauftragErneut(versandauftragDto.getIId(),
					null);
			...
			DelegateFactory.getInstance().getVersandDelegate().sendeVersandauftragSofort(versandauftragDto.getIId());
			// wirkt fuer diesen Filter wie Loeschen -> QP informieren
			ItemChangedEvent it = new ItemChangedEvent(this, ItemChangedEvent.ACTION_GOTO_MY_DEFAULT_QP);
			this.tpVersandauftrag.lPEventItemChanged(it);
		}
	...
	public void eventActionNew(EventObject e, boolean bChangeKeyLockMeI, boolean bNeedNoNewI) throws Throwable {
		...
		PartnerDto partnerDtoAbsender = DelegateFactory.getInstance().getPartnerDelegate()
				.partnerFindByPrimaryKey(personalDto.getPartnerIId());
		PartnerkommunikationDto[] komm = DelegateFactory.getInstance().getPartnerDelegate()
				.partnerkommFindByPartnerIIdKommunikationsartPAiIdKommArtMandant(partnerDtoAbsender.getIId(),
						PartnerFac.KOMMUNIKATIONSART_EMAIL, LPMain.getTheClient().getMandant());
	...
	protected void eventActionPrint(ActionEvent e) throws Throwable {
		...
		getInternalFrame().showReportKriterien(new ReportVersandauftrag(getInternalFrame(), versandauftragDto, null),
				null, null, false, false, false);

		/*
		 * getInternalFrame().showReportKriterien(new
		 * ReportVersandauftrag(getInternalFrame(), versandauftragDto, "my title"),
		 * empfaengerPartnerDto, empfaengerPartnerDto.getIId(), false);
		 */
	...
	private boolean isHtml() {
		...
		String mailtext = versandauftragDto.getCText();
		if (Helper.isStringEmpty(mailtext))
			return false;

		return mailtext.startsWith("<html") || mailtext.startsWith("<HTML") || mailtext.startsWith("<!DOCTYPE");
	}

	private void addTextPanelToGrid(boolean isHtml) {
		if (componentText != null) {
			if (isHtml && componentText == wefHtmlText)
				return;

			if (!isHtml && componentText == wefText)
				return;

			jPanelWorkingOn.remove(componentText);
		}

		componentText = isHtml ? wefHtmlText : wefText;
		jPanelWorkingOn.add(componentText, new GridBagConstraints(1, componentTextRow, 5, 1, 0.0, 1.0,
				GridBagConstraints.CENTER, GridBagConstraints.BOTH, new Insets(2, 2, 2, 2), 0, 0));
	}

	private void switchTextPanels(boolean isHtml) throws Throwable {
