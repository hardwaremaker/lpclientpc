...
public class NettoEinzelpreisMinusRabatteKorrigieren6402 extends PflegefunktionSupportsProgressBar {
	...
	public String getName() {
		return "Artikelset Nettoeinzelpreiseminusrabatte SP6402";
	}
	...
	public String getBeschreibung() {
		...
		return "<html>" +
				"In Rechnungs- und Lieferscheinpositionen die zu einem Artikelset geh&ouml;ren " +
				"die Preisstatistikfelder ab Gesch&auml;ftsjahr 2017 korrigieren." +
				(clientCount > 1
...
//		console.setVisible(false) ;
	...
	public void run() {
		...
		try {
			...
			List<Integer> allInvoices = DelegateFactory.getInstance().getRechnungDelegate().repairRechnungSP6402GetList() ;
			List<Integer> allDeliveries = DelegateFactory.getInstance().getLsDelegate().repairLieferscheinSP6402GetList();
			NettoEinzelpreisMinusRabatteKorrigieren6402.this.setProgress(new DefaultBoundedRangeModel(0, 0, 0, allInvoices.size() + allDeliveries.size()));
...
//					setProgress(++count);
					...
					DelegateFactory.getInstance().getRechnungDelegate().repairRechnungSP6402(rechnungId);
...
//					setProgress(++count);
					...
					DelegateFactory.getInstance().getLsDelegate().repairLieferscheinSP6402(lieferscheinId) ;
		...
		protected void process(List<Workerdata> chunks) {
			int count = NettoEinzelpreisMinusRabatteKorrigieren6402.this.getProgress().getValue() ;
