...
public class DialogBestellvorschlagImportXLS extends JDialog implements ActionListener {
	/**
	 * 
	 */
	...
	private WrapperCheckBox wcbVorhandenenBVLoeschen = new WrapperCheckBox();

	private WrapperLabel wlaLiefertermin = new WrapperLabel();
	private WrapperDateField wdfLiefertermin = new WrapperDateField();
	...
	private TabbedPaneBestellvorschlag tpBV = null;

	public DialogBestellvorschlagImportXLS(byte[] xlsDatei, TabbedPaneBestellvorschlag tpBV) throws Throwable {
		super(LPMain.getInstance().getDesktop(), "Bestellvorschlag XLS importieren", true);
	...
	private void jbInit() throws Throwable {
		...
		wtaFehler.setText(DelegateFactory.getInstance().getBestellvorschlagDelegate()
				.pruefeUndImportiereBestellvorschlagXLS(xlsDatei, wdfLiefertermin.getTimestamp(), false, false));

		wbuAbbrechen.setText(LPMain.getInstance().getTextRespectUISPr("lp.abbrechen"));

		wcbVorhandenenBVLoeschen.setText(LPMain.getInstance().getTextRespectUISPr("bv.import.xls.vorhandenenloeschen"));
		...
		wlaLiefertermin.setText(LPMain.getTextRespectUISPr("bv.import.xls.liefertermin"));
		...
		panelUrlaubsanspruch.add(wlaLiefertermin, new GridBagConstraints(0, 2, 1, 1, 1, 0.0, GridBagConstraints.CENTER,
				GridBagConstraints.HORIZONTAL, new Insets(2, 2, 2, 2), 0, 0));
		panelUrlaubsanspruch.add(wdfLiefertermin, new GridBagConstraints(1, 2, 1, 1, 1, 0.0, GridBagConstraints.CENTER,
				GridBagConstraints.HORIZONTAL, new Insets(2, 2, 2, 2), 0, 0));

		panelUrlaubsanspruch.add(wcbVorhandenenBVLoeschen, new GridBagConstraints(0, 3, 2, 1, 0.0, 0.0,
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource().equals(wbuImportieren)) {
			try {
				...
				if (wdfLiefertermin.getTimestamp() == null) {
				...
				String fehler = DelegateFactory.getInstance().getBestellvorschlagDelegate()
						.pruefeUndImportiereBestellvorschlagXLS(xlsDatei,

								wdfLiefertermin.getTimestamp(), wcbVorhandenenBVLoeschen.isSelected(), true);
				...
				tpBV.getPanelBestellungVorschlagSP1().handleException(e2, true);
			}
