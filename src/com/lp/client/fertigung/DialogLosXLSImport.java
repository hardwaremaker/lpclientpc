...
public class DialogLosXLSImport extends JDialog implements ActionListener {
	/**
	 * 
	 */
	...
	ImportPruefergebnis importPruefergebnis = null;
	...
	public DialogLosXLSImport(byte[] xlsDatei, TabbedPaneLos tpPartner)
			throws Throwable {
		super(LPMain.getInstance().getDesktop(), "Los XLS-Import", true);
	...
	private void jbInit() throws Throwable {
		...
		importPruefergebnis = DelegateFactory.getInstance()
				.getFertigungDelegate().pruefeLosimportXLS(xlsDatei);

		ArrayList<String> alFehler = importPruefergebnis.getAlFehler();

		byte[] CRLFAscii = { 13, 10 };
		String sFehlerUndWarnungen = "";
		if (alFehler.size() > 0) {
			sFehlerUndWarnungen += LPMain.getInstance().getTextRespectUISPr(
					"fert.xlsimport.fehler")
					+ new String(CRLFAscii) + new String(CRLFAscii);

			for (int i = 0; i < alFehler.size(); i++) {
				sFehlerUndWarnungen += alFehler.get(i) + new String(CRLFAscii);
			}

		}
		sFehlerUndWarnungen += new String(CRLFAscii);
		ArrayList<String> alWarnung = importPruefergebnis.getAlWarnung();
		if (alWarnung.size() > 0) {
			sFehlerUndWarnungen += LPMain.getInstance().getTextRespectUISPr(
					"fert.xlsimport.warnung")
					+ new String(CRLFAscii) + new String(CRLFAscii);

			for (int i = 0; i < alWarnung.size(); i++) {
				sFehlerUndWarnungen += alWarnung.get(i) + new String(CRLFAscii);
			}

		}

		wtaFehler.setText(sFehlerUndWarnungen);
		...
		if (importPruefergebnis.hatFehler() == true) {
			wbuImportieren.setEnabled(false);
		} else {
			if (importPruefergebnis.hatWarnings() == false) {
		...
		if (e.getSource().equals(wbuImportieren)) {
			try {
				DelegateFactory.getInstance().getFertigungDelegate()
						.importiereLoseXLS(importPruefergebnis);

				this.setVisible(false);

			} catch (Throwable e2) {
				tpLos.handleException(e2, true);
			}
