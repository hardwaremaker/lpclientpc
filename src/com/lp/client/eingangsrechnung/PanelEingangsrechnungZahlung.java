...
//	private TabbedPaneEingangsrechnung tabbedPaneRechnungAll = null;
	...
	public PanelEingangsrechnungZahlung(InternalFrame internalFrame, String add2TitleI,
			TabbedPaneEingangsrechnung tabbedPaneRechnungAll) throws Throwable {
...
//		return tabbedPaneRechnungAll;
	...
	private void jbInit() throws Throwable {
		ParametermandantDto para = DelegateFactory.getInstance().getParameterDelegate().getMandantparameter(
		...
		if (bMitKurseingabe) {
			...
			wcbKursUebersteuert.setText(LPMain.getTextRespectUISPr("label.zahlung.kursuebersteuert"));
			wcbKursUebersteuert.setActionCommand(ACTION_SPECIAL_KURSUEBERSTEUERT);
	...
	private void dto2Components() throws Throwable {
		if (getKeyWhenDetailPanel() != null) {
			zahlungDto = DelegateFactory.getInstance().getEingangsrechnungDelegate()
					.eingangsrechnungzahlungFindByPrimaryKey((Integer) getKeyWhenDetailPanel());
		}
		if (zahlungDto != null) {
			// sub-dtos
			...
			if (zahlungsartCNr.equalsIgnoreCase(RechnungFac.ZAHLUNGSART_BANK)) {
				super.bankverbindungDto = DelegateFactory.getInstance().getFinanzDelegate()
						.bankverbindungFindByPrimaryKey(zahlungDto.getBankverbindungIId());
				...
				if (zahlungDto.getRechnungzahlungIId() != null) {
					RechnungzahlungDto dto = DelegateFactory.getInstance().getRechnungDelegate()
							.zahlungFindByPrimaryKey(zahlungDto.getRechnungzahlungIId());
				...
				if (zahlungDto.getEingangsrechnungIIdGutschriftZahlung() != null) {
					erGutschriftZahlungDto = DelegateFactory.getInstance().getEingangsrechnungDelegate()
							.eingangsrechnungzahlungFindByPrimaryKey(
									zahlungDto.getEingangsrechnungIIdGutschriftZahlung());
				}
				if (zahlungDto.getEingangsrechnungIIdGutschrift() != null) {
					erGutschriftDto = DelegateFactory.getInstance().getEingangsrechnungDelegate()
							.eingangsrechnungFindByPrimaryKey(zahlungDto.getEingangsrechnungIIdGutschrift());
			...
			if (bMitKurseingabe) {
				...
				if (wDto != null) {
					aktuelleKurs = wDto.getNKurs().setScale(6, BigDecimal.ROUND_HALF_EVEN);
				}
	...
	public void eventActionNew(EventObject e, boolean bChangeKeyLockMeI, boolean bNeedNoNewI) throws Throwable {
		...
		// PJ16076
		ArrayList<EingangsrechnungDto> al = DelegateFactory.getInstance().getEingangsrechnungDelegate()
				.eingangsrechnungFindOffeneByLieferantIId(getEingangsrechnungDto().getLieferantIId());

		if (al.size() > 0) {
			String s = "\n";
			for (int i = 0; i < al.size(); i++) {
				EingangsrechnungDto erDto = al.get(i);
				s += erDto.getCNr() + " " + LPMain.getTextRespectUISPr("lp.bruttowert") + " "
						+ Helper.formatZahl(erDto.getNBetragfw(), 2, LPMain.getTheClient().getLocUi()) + " "
						+ erDto.getWaehrungCNr() + "\n";
			}

			DialogFactory.showModalDialog(LPMain.getTextRespectUISPr("lp.info"),
					LPMain.getTextRespectUISPr("er.offenegutschriften.vorhanden") + s);
		...
		if (this.zahlungDto != null) {
			if (zahlungDto.getIId() != null) {
				if (!isLockedDlg()) {
					...
					if (wcoZahlungsart.getSelectedItem().equals(RechnungFac.ZAHLUNGSART_GEGENVERRECHNUNG)) {
						DialogFactory.showModalDialog(LPMain.getTextRespectUISPr("lp.error"),
								LPMain.getTextRespectUISPr("er.zahlung.error.gegenverrechnung"));
						return;
					}
					...
					if (getTabbedPane().getEingangsrechnungDto().getStatusCNr()
						...
						boolean answer = (DialogFactory.showMeldung(
								LPMain.getTextRespectUISPr("er.erledigtzahlungloeschen"),
					...
					if (zahlungDto.getZahlungsartCNr().equals(RechnungFac.RECHNUNGART_GUTSCHRIFT)) {
						DelegateFactory.getInstance().getEingangsrechnungDelegate()
								.removeEingangsrechnungzahlung(erGutschriftZahlungDto);
					}
					this.zahlungDto = null;
	...
	public void eventActionSave(ActionEvent e, boolean bNeedNoSaveI) throws Throwable {
		if (allMandatoryFieldsSetDlg()) {
			...
			if (wnfOffen.getBigDecimal() != null && wnfOffen.getBigDecimal().compareTo(new BigDecimal(0)) < 0) {
			...
			if (bErledigt && wnfOffen.getBigDecimal() != null
			...
			if (bSpeichern) {
				...
				if (zahlungDto.getZahlungsartCNr().equals(RechnungFac.RECHNUNGART_GUTSCHRIFT)) {
					...
					erGutschriftZahlungDto.setEingangsrechnungIId(erGutschriftDto.getIId());
					// Bei einer Gutschrift wird der FK auf die Gutschrift fuer
					// Die zugehoerige Rechnung verwendet
					erGutschriftZahlungDto.setEingangsrechnungIIdGutschrift(zahlungDto.getEingangsrechnungIId());
					erGutschriftZahlungDto.setTZahldatum(zahlungDto.getTZahldatum());
					erGutschriftZahlungDto.setZahlungsartCNr(RechnungFac.ZAHLUNGSART_GUTSCHRIFT);
					erGutschriftZahlungDto.setNBetrag(zahlungDto.getNBetrag().multiply(bdMinus));
					erGutschriftZahlungDto.setNBetragfw(zahlungDto.getNBetragfw().multiply(bdMinus));
					erGutschriftZahlungDto.setNBetragust(zahlungDto.getNBetragust().multiply(bdMinus));
					erGutschriftZahlungDto.setNBetragustfw(zahlungDto.getNBetragustfw().multiply(bdMinus));
					erGutschriftZahlungDto.setNKurs(zahlungDto.getNKurs());
					erGutschriftZahlungDto.setBKursuebersteuert(zahlungDto.getBKursuebersteuert());
					erGutschriftZahlungDto.setPersonalIIdAendern(zahlungDto.getPersonalIIdAendern());
					erGutschriftZahlungDto.setPersonalIIdAnlegen(zahlungDto.getPersonalIIdAnlegen());
					erGutschriftZahlungDto.setTAnlegen(new Timestamp(Calendar.getInstance().getTime().getTime()));
					erGutschriftZahlungDto.setTAendern(new Timestamp(Calendar.getInstance().getTime().getTime()));
					erGutschriftZahlungDto.setPersonalIIdAendern(zahlungDto.getPersonalIIdAendern());
					erGutschriftZahlungDto.setPersonalIIdAnlegen(zahlungDto.getPersonalIIdAnlegen());

					erGutschriftZahlungDto.setCKommentar(zahlungDto.getCKommentar());

					BigDecimal bdBereitsBezahlt = DelegateFactory.getInstance().getEingangsrechnungDelegate()
							.getBezahltBetragFw(erGutschriftDto.getIId(), null);

					bdBereitsBezahlt = bdBereitsBezahlt.negate().add(erGutschriftZahlungDto.getNBetragfw().negate());

					if (bdBereitsBezahlt.add(erGutschriftDto.getNBetragfw()).signum() > 0) {
						DialogFactory.showModalDialog(LPMain.getTextRespectUISPr("lp.error"),
								LPMain.getTextRespectUISPr("rech.error.geutschriftzuniedrig"));
						return;
					}

					if (bdBereitsBezahlt.doubleValue() >= erGutschriftDto.getNBetragfw().negate().doubleValue()) {
						erGutschriftDto.setStatusCNr(EingangsrechnungFac.STATUS_ERLEDIGT);
						erGutschriftZahlungDto = DelegateFactory.getInstance().getEingangsrechnungDelegate()
								.updateEingangsrechnungzahlung(erGutschriftZahlungDto, true);

					} else {
						erGutschriftDto.setStatusCNr(EingangsrechnungFac.STATUS_TEILBEZAHLT);
						erGutschriftZahlungDto = DelegateFactory.getInstance().getEingangsrechnungDelegate()
								.updateEingangsrechnungzahlung(erGutschriftZahlungDto, false);

					}

					zahlungDto.setEingangsrechnungIIdGutschriftZahlung(erGutschriftZahlungDto.getIId());
					zahlungDto.setEingangsrechnungIIdGutschrift(erGutschriftDto.getIId());
				}
				zahlungDto = DelegateFactory.getInstance().getEingangsrechnungDelegate()
				...
				if (getInternalFrame().getKeyWasForLockMe() == null) {
					...
					getInternalFrame().setKeyWasForLockMe(zahlungDto.getEingangsrechnungIId().toString());
				}
	...
	private void components2Dto() throws Throwable {
		...
		if (bMitKurseingabe && wcbKursUebersteuert.isSelected()) {
			...
			zahlungDto.setNBetrag(wnfBetrag.getBigDecimal().divide(zahlungDto.getNKurs(), FinanzFac.NACHKOMMASTELLEN,
					BigDecimal.ROUND_HALF_EVEN));
			zahlungDto.setNBetragust(wnfBetragUST.getBigDecimal().divide(zahlungDto.getNKurs(),
			...
			zahlungDto.setNBetrag(
					DelegateFactory.getInstance().getLocaleDelegate().rechneUmInAndereWaehrungGerundetZuDatum(
							wnfBetrag.getBigDecimal(), waehrung, mandantWaehrung, wdfDatum.getDate()));
			zahlungDto.setNBetragust(
					DelegateFactory.getInstance().getLocaleDelegate().rechneUmInAndereWaehrungGerundetZuDatum(
							wnfBetragUST.getBigDecimal(), waehrung, mandantWaehrung, wdfDatum.getDate()));
			...
			if (mandantWaehrung.equals(waehrung)) {
				...
				WechselkursDto kursDto = DelegateFactory.getInstance().getLocaleDelegate()
						.getKursZuDatum(mandantWaehrung, waehrung, wdfDatum.getDate());
	...
	private void dto2ComponentsRechnung() throws Throwable {
		...
		if (this.getLockedstateDetailMainKey().getIState() != PanelBasis.LOCK_FOR_NEW
			...
			wcbErledigt.setSelected(rechnungDto.getStatusCNr().equals(EingangsrechnungFac.STATUS_ERLEDIGT));
		}
	}

	private void setDefaults() throws Throwable {
		boolean bSchlussrechnung = false;
		if (getEingangsrechnungDto().getEingangsrechnungartCNr()
	...
	protected void dialogQueryGutschriftER(ActionEvent e, Integer LieferantIId) throws Throwable {
		// ffcreatespanel: fuer eine dialogquery
		panelQueryFLRGutschrift = FinanzFilterFactory.getInstance().createPanelFLRGutschriftER(getInternalFrame(),
				LieferantIId);
	...
	protected void eventActionSpecial(ActionEvent e) throws Throwable {
		...
		if (e.getActionCommand().equals(ACTION_SPECIAL_GUTSCHRIFT)) {
			dialogQueryGutschriftER(e, getEingangsrechnungDto().getLieferantIId());
	...
	private BigDecimal getKursAktuell() throws ExceptionLP, Throwable {
		...
		if (rechnungDto.getWaehrungCNr().equals(mandantWaehrung)) {
			...
			WechselkursDto kursDto = DelegateFactory.getInstance().getLocaleDelegate().getKursZuDatum(mandantWaehrung,
	...
	protected void eventItemchanged(EventObject eI) throws Throwable {
		...
		if (e.getID() == ItemChangedEvent.GOTO_DETAIL_PANEL) {
			if (e.getSource() == panelQueryFLRGutschrift) {
				...
				if (erGutschriftZahlungDto != null) {
					bdBezahlt = DelegateFactory.getInstance().getEingangsrechnungDelegate()
							.getBezahltBetragFw(erGutschriftDto.getIId(), erGutschriftZahlungDto.getIId());
				} else {
					bdBezahlt = DelegateFactory.getInstance().getEingangsrechnungDelegate()
							.getBezahltBetragFw(erGutschriftDto.getIId(), null);

				}
				BigDecimal diff = erGutschriftDto.getNBetragfw().subtract(bdBezahlt);
				BigDecimal gesOffen = getWertGesamtOffenExklusiveZahlung();
				wnfBetrag
						.setBigDecimal(diff.negate().doubleValue() > gesOffen.doubleValue() ? gesOffen : diff.negate());
				focusLostWnfBetrag();
			}
		}
	...
	public void eventYouAreSelected(boolean bNeedNoYouAreSelectedI) throws Throwable {
		...
		if (key == null || (key != null && key.equals(LPMain.getLockMeForNew()))) {
			...
			if (EingangsrechnungFac.EINGANGSRECHNUNGART_GUTSCHRIFT
					.equals(getTabbedPane().getEingangsrechnungDto().getEingangsrechnungartCNr())) {
	...
	protected BigDecimal getWertGesamtOffenExklusiveZahlung() throws Throwable {
		EingangsrechnungDto erDto = getTabbedPane().getEingangsrechnungDto();
		return erDto.getNBetragfw().subtract(getEingangsrechnungDelegate().getBezahltBetragFw(erDto.getIId(), null))
				.add(zahlungDto == null ? new BigDecimal(0) : zahlungDto.getNBetragfw());
	}
	...
	protected BigDecimal getWertGesamtOffenUstExklusiveZahlung() throws Throwable {
		...
		return erDto.getNUstBetragfw()
				.subtract(getEingangsrechnungDelegate().getBezahltBetragUstFw(erDto.getIId(), null))
				.add(zahlungDto == null ? new BigDecimal(0) : zahlungDto.getNBetragustfw());
	}

	protected BigDecimal getWertBereitsBezahlt() throws Throwable {
		EingangsrechnungDto erDto = getTabbedPane().getEingangsrechnungDto();
		if (erDto == null || erDto.getIId() == null) {
			return BigDecimal.ZERO;
		}

		BigDecimal betragAbZahlung = zahlungDto != null ? getEingangsrechnungDelegate()
				.getBezahltBetragFw(erDto.getIId(), zahlungDto.getIId()).add(zahlungDto.getNBetragfw())
				: new BigDecimal(0);
		BigDecimal gesamtBezahlt = getEingangsrechnungDelegate().getBezahltBetragFw(erDto.getIId(), null);
	...
	protected BigDecimal getWertBereitsBezahltUst() throws Throwable {
		...
		BigDecimal betragAbZahlung = zahlungDto != null ? getEingangsrechnungDelegate()
				.getBezahltBetragUstFw(erDto.getIId(), zahlungDto.getIId()).add(zahlungDto.getNBetragustfw())
				: new BigDecimal(0);
		BigDecimal gesamtBezahlt = getEingangsrechnungDelegate().getBezahltBetragUstFw(erDto.getIId(), null);
		return gesamtBezahlt.subtract(betragAbZahlung);
	}
	...
	protected BigDecimal getWertAnzahlungen() throws Throwable {
		...
		if (erDto.getEingangsrechnungartCNr().equals(EingangsrechnungFac.EINGANGSRECHNUNGART_SCHLUSSZAHLUNG))
			return getEingangsrechnungDelegate().getAnzahlungenZuSchlussrechnungFw(erDto.getIId());
		else
	...
	protected BigDecimal getWertAnzahlungenUst() throws Throwable {
		EingangsrechnungDto erDto = getTabbedPane().getEingangsrechnungDto();
		if (erDto.getEingangsrechnungartCNr().equals(EingangsrechnungFac.EINGANGSRECHNUNGART_SCHLUSSZAHLUNG))
			return getEingangsrechnungDelegate().getAnzahlungenZuSchlussrechnungUstFw(erDto.getIId());
		else
			return new BigDecimal(0);
	}
	...
	protected java.sql.Date getDBelegdatum() {
		return new java.sql.Date(getTabbedPane().getEingangsrechnungDto().getDBelegdatum().getTime());
	}
	...
	private EingangsrechnungDelegate getEingangsrechnungDelegate() throws Throwable {
		return DelegateFactory.getInstance().getEingangsrechnungDelegate();
	}

	@Override
	protected Integer getKontoIIdAnzahlung() throws Throwable {
		LieferantDto lieferant = DelegateFactory.getInstance().getLieferantDelegate()
	...
	protected boolean isGutschriftErlaubt() {
		if (getEingangsrechnungDto().isEingangsrechnung() && getEingangsrechnungDto().getNBetrag().signum() < 0) {
			return false;
		}

		return !getEingangsrechnungDto().isAnzahlung();
	}

	@Override
	protected boolean isVorauszahlungErlaubt() {
		if (getEingangsrechnungDto().isAnzahlung()) {
			return false;
		}
...
//		}
	...
	protected void initializeGutschriftButton(String zahlungartCnr) {
		if (RechnungFac.ZAHLUNGSART_GUTSCHRIFT.equalsIgnoreCase(zahlungartCnr)
				&& erGutschriftDto.getEingangsrechnungartCNr()
						.equalsIgnoreCase(EingangsrechnungFac.EINGANGSRECHNUNGART_EINGANGSRECHNUNG)) {
