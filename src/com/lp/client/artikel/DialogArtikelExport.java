...
import com.lp.client.frame.filechooser.filter.HvTaggedXlsFilter;
...
public class DialogArtikelExport extends JDialog implements ActionListener, ItemChangedListener {
	...
	private WrapperCheckBox wcbMitUntergruppen = null;
	private WrapperCheckBox wcbMitUnterklassen = null;
	private WrapperCheckBox wcbMitUntershopgruppen = null;
	...
	public DialogArtikelExport(InternalFrameArtikel internalFrame) throws Throwable {
		super(LPMain.getInstance().getDesktop(), LPMain.getInstance().getTextRespectUISPr("artikel.preispflege.export"),
				true);
...
//		this.setSize(Defaults.getInstance().bySizeFactor(500), Defaults.getInstance().bySizeFactor(200));
		...
		if (e.getSource().equals(btnOK)) {
			...
			try {
				...
				HvOptional<WrapperFile> wf = HelperClient.showSaveDialog(
						...
						new File("vkpreise.xls"), new HvTaggedXlsFilter());
				if (wf.isPresent()) {
					byte[] datenXlsFile = DelegateFactory.getInstance().getArtikelDelegate().getXLSForPreispflege(
							wsfArtikelgruppe.getIKey(), wcbMitUntergruppen.isSelected(), wsfArtikelklasse.getIKey(),
							wcbMitUnterklassen.isSelected(), wsfShopgruppe.getIKey(), wcbMitUntershopgruppen.isSelected(),
							wtfArtikelnrVon.getText(), wtfArtikelnrBis.getText(), wcbVersteckte.isSelected());

					String path = wf.get().getFile().getPath();
					...
					DialogFactory.showModalDialog(LPMain.getTextRespectUISPr("lp.hinweis"),
							LPMain.getTextRespectUISPr("lp.hint.dateiwurdegespeichert") + " (" + file.getAbsolutePath()
									+ ") ");

				}
			} catch (FileNotFoundException e2) {

				DialogFactory.showModalDialog(LPMain.getTextRespectUISPr("lp.hinweis"), e2.getMessage());
				return;
			} catch (Throwable e1) {
	...
	void dialogQueryArtikelFromListe_Von(ActionEvent e) throws Throwable {
		panelQueryFLRArtikel_Von = ArtikelFilterFactory.getInstance().createPanelFLRArtikel(internalFrame,
				artikelIId_Von, true);
	...
	void dialogQueryArtikelFromListe_Bis(ActionEvent e) throws Throwable {
		panelQueryFLRArtikel_Bis = ArtikelFilterFactory.getInstance().createPanelFLRArtikel(internalFrame,
				artikelIId_Bis, true);
	...
	private void jbInit() throws Throwable {
		...
		wsfArtikelgruppe = new WrapperSelectField(WrapperSelectField.ARTIKELGRUPPE, internalFrame, true);
		wsfArtikelklasse = new WrapperSelectField(WrapperSelectField.ARTIKELKLASSE, internalFrame, true);

		wsfShopgruppe = new WrapperSelectField(WrapperSelectField.SHOPGRUPPE, internalFrame, true);

		
		wcbMitUntergruppen=new WrapperCheckBox(LPMain.getTextRespectUISPr("artikel.preislisteexport.mituntergruppen"));
		wcbMitUnterklassen=new WrapperCheckBox(LPMain.getTextRespectUISPr("artikel.preislisteexport.mitunterklassen"));
		wcbMitUntershopgruppen=new WrapperCheckBox(LPMain.getTextRespectUISPr("artikel.preislisteexport.mituntershopgruppen"));
		...
		this.getContentPane().add(wcbMitUntergruppen,"wrap");

		this.getContentPane().add(wsfArtikelklasse.getWrapperButton());
		this.getContentPane().add(wsfArtikelklasse.getWrapperTextField());
		this.getContentPane().add(wcbMitUnterklassen,"wrap");

		this.getContentPane().add(wsfShopgruppe.getWrapperButton());
		this.getContentPane().add(wsfShopgruppe.getWrapperTextField());
		this.getContentPane().add(wcbMitUntershopgruppen,"wrap");
