 ...
 */
public class PanelDialogPreispflege extends PanelDialogKriterien {
	/**
	 * 
	 */
	...
	private static final String ACTION_SPECIAL_FLR_PREISLISTE = "ACTION_SPECIAL_FLR_PREISLISTE";

	public PanelDialogPreispflege(InternalFrame p0, String title) throws Throwable {
		super(p0, title);
		jbInit();
		initComponents();
	}
	...
	private void jbInit() throws Throwable {
		...
		wlaProzentsatz.setText("VK-Preisbasis/ Fixpreise \u00E4ndern um");
		wbuArtikelgruppe.setText(LPMain.getInstance().getTextRespectUISPr("button.artikelgruppe"));

		wbuPreisliste.setText(LPMain.getInstance().getTextRespectUISPr("vkpf.button.preislisten"));
		...
		jpaWorkingOn.add(wbuArtikelgruppe, new GridBagConstraints(0, 0, 1, 1, 0, 0.0, GridBagConstraints.CENTER,
				GridBagConstraints.BOTH, new Insets(2, 2, 2, 2), 0, 0));
		jpaWorkingOn.add(wtfArtikelgruppe, new GridBagConstraints(1, 0, 2, 1, 0, 0.0, GridBagConstraints.CENTER,
				GridBagConstraints.BOTH, new Insets(2, 2, 2, 2), 0, 0));
		jpaWorkingOn.add(wbuPreisliste, new GridBagConstraints(0, 1, 1, 1, 0, 0.0, GridBagConstraints.CENTER,
				GridBagConstraints.BOTH, new Insets(2, 2, 2, 2), 0, 0));
		jpaWorkingOn.add(wtfPreisliste, new GridBagConstraints(1, 1, 2, 1, 0, 0.0, GridBagConstraints.CENTER,
				GridBagConstraints.BOTH, new Insets(2, 2, 2, 2), 0, 0));
		...
		JLabel wlaHinweis = new JLabel();

		wlaHinweis.setText(LPMain.getInstance().getTextRespectUISPr("artikel.vkpreise.pflege.hinweis"));
		
		
		
		//wlaHinweis.setText(
			//	"<html>Hinweis:<br/>Wenn eine Preisliste augewählt wurde, werden die VK-Preisbasis und zusätzlich alle Fixpreise um den gewählten Prozentsatz erhöht.<br> Rabattsätze in der Preisliste bleiben unberührt.</html> ");

		jpaWorkingOn.add(wlaHinweis, new GridBagConstraints(0, 4, 2, 2, 0, 0.0, GridBagConstraints.CENTER,
				GridBagConstraints.BOTH, new Insets(20, 2, 2, 2), 0, 50));
	...
	protected void eventItemchanged(EventObject eI) throws Throwable {
		...
		if (e.getID() == ItemChangedEvent.GOTO_DETAIL_PANEL) {
			if (e.getSource() == panelQueryFLRArtikelgruppe) {
				...
				Integer iIdPreisliste = (Integer) ((ISourceEvent) e.getSource()).getIdSelected();
				VkpfartikelpreislisteDto vkpfartikelpreislisteDto = DelegateFactory.getInstance()
						.getVkPreisfindungDelegate().vkpfartikelpreislisteFindByPrimaryKey(iIdPreisliste);
	...
	protected void eventActionSpecial(ActionEvent e) throws Throwable {
		if (e.getActionCommand().equals(ACTION_SPECIAL_OK)) {
			if (allMandatoryFieldsSetDlg()) {
				...
				DelegateFactory.getInstance().getVkPreisfindungDelegate().pflegeVkpreise(selectedArtikelgruppeIId,
						selectedVkpfpreislisteIId, wdfAendernAb.getDate(), wnfProzentsatz.getBigDecimal(), bemerkung);
			}
			...
			panelQueryFLRPreisliste = ArtikelFilterFactory.getInstance().createPanelFLRPreisliste(getInternalFrame(),
					selectedVkpfpreislisteIId);
