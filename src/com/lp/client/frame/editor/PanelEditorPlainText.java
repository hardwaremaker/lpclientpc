...
public class PanelEditorPlainText extends PanelDialog {
	/**
	 *
	 */
	private static final long serialVersionUID = 1L;
	private WrapperTextField targetComponent = null;
	...
	public PanelEditorPlainText(InternalFrame internalFrame,
			WrapperTextField targetComponent, String addTitleI, String text,
	...
	public void setColumnsMax(int iColumnsMax){
		lpEditor.getTextBlockAttributes(-1).capacity =iColumnsMax;
	}
	
	private void setupEditor(WrapperTextField targetComponent, String text)
			throws FontNotFoundException, ExceptionLP, Throwable {
		lpEditor = new LpEditor(null);
		lpEditor.setInternalFrame(getInternalFrame());
		// lpEditor.setJasperReport(DelegateFactory.getInstance().getSystemDelegate().
		// getDreispalter(), false);
		lpEditor.getTextBlockAttributes(-1).capacity = targetComponent.getColumnsMax();
		lpEditor.showAlignmentItems(false);
		lpEditor.showFileItems(false);
		lpEditor.showFontStyleItems(true);
		lpEditor.showMenu(false);
	...
	protected void eventActionSpecial(ActionEvent e) throws Throwable {
		...
		try {
			sNewText = lpEditor.getPlainText();
