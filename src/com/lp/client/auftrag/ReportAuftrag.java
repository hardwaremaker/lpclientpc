 ...
 */
public class ReportAuftrag extends ReportBeleg implements IDruckTypeReport {
	/**
	 *
	 */
...
//						wnfKopien.getInteger(),
	...
	public MailtextDto getMailtextDto() throws Throwable {
		...
		if (auftragDto != null) {
			...
			mailtextDto.setProjektIId(auftragDto.getProjektIId());
			/**
			...
			mailtextDto.setBelegVersion(auftragDto.getIVersion());
			mailtextDto.setMailFusstext(getAuftragFusstext());
			mailtextDto.setMailKopftext(getAuftragKopftext());
		}
		
		return mailtextDto;
	}
	
	private String getAuftragKopftext() throws ExceptionLP {
		HvOptional<String> kopftext = HvOptional.ofNullable(auftragDto.getCKopftextUebersteuert());
		if (kopftext.isPresent())
			return kopftext.get();
		
		
		HvOptional<AuftragtextDto> defaultKopftextDto = HvOptional.ofNullable(
				DelegateFactory.getInstance().getAuftragServiceDelegate()
					.getAufragkopfDefault(kundeDto.getPartnerDto().getLocaleCNrKommunikation()));
		return defaultKopftextDto.isPresent()
				? defaultKopftextDto.get().getCTextinhalt()
				: null;
	}
	
	private String getAuftragFusstext() throws ExceptionLP {
		HvOptional<String> fusstext = HvOptional.ofNullable(auftragDto.getCFusstextUebersteuert());
		if (fusstext.isPresent())
			return fusstext.get();
		
		HvOptional<AuftragtextDto> defaultFusstextDto = HvOptional.ofNullable(
				DelegateFactory.getInstance().getAuftragServiceDelegate()
					.getAufragfussDefault(kundeDto.getPartnerDto().getLocaleCNrKommunikation()));
		return defaultFusstextDto.isPresent()
			? defaultFusstextDto.get().getCTextinhalt()
			: null;
	}
