 ...
 */
public class PanelAuftragTeilnehmer extends PanelBasis {
	/**
	 * 
	 */
	...
	static final public String ACTION_SPECIAL_TEILNEHMER = "action_special_personal_teilnehmer";
	static final public String ACTION_SPECIAL_TEILNEHMERFUNKTION = "action_special_teilnehmerfunktion";
	...
	public PanelAuftragTeilnehmer(InternalFrame internalFrame, String add2TitleI, Object key) throws Throwable {
	...
	void jbInit() throws Throwable {
		...
		wbuTeilnehmer = new WrapperButton();
		wbuTeilnehmer.setText(LPMain.getTextRespectUISPr("button.teilnehmer"));
		wbuTeilnehmer.addActionListener(this);
		wbuTeilnehmer.setActionCommand(ACTION_SPECIAL_TEILNEHMER);

		wtfTeilnehmer = new WrapperTextField();
		wtfTeilnehmer.setMandatoryFieldDB(true);
		wtfTeilnehmer.setActivatable(false);

		wbuTeilnehmerfunktion = new WrapperButton(LPMain.getTextRespectUISPr("teiln.label.funktion"));
		wbuTeilnehmerfunktion.addActionListener(this);
		wbuTeilnehmerfunktion.setActionCommand(ACTION_SPECIAL_TEILNEHMERFUNKTION);
		...
		wlaAngelegt = new WrapperLabel(LPMain.getTextRespectUISPr("teiln.label.erfasst"));
		...
		wcbExternerTeilnehmer.setText(LPMain.getTextRespectUISPr("auft.teiln.externerteilnehmer"));
		wcbExternerTeilnehmer.setActivatable(false);

		jPanelWorkingOn.add(wbuTeilnehmer, new GridBagConstraints(0, 0, 1, 1, 0.0, 0.0, GridBagConstraints.CENTER,
				GridBagConstraints.BOTH, new Insets(2, 2, 2, 2), 0, 0));
		jPanelWorkingOn.add(wtfTeilnehmer, new GridBagConstraints(1, 0, 4, 1, 0.0, 0.0, GridBagConstraints.CENTER,
				GridBagConstraints.BOTH, new Insets(2, 2, 2, 2), 0, 0));
		jPanelWorkingOn.add(wbuTeilnehmerfunktion, new GridBagConstraints(0, 1, 1, 1, 0.0, 0.0,
				GridBagConstraints.CENTER, GridBagConstraints.BOTH, new Insets(2, 2, 2, 2), 0, 0));
		jPanelWorkingOn.add(wtfTeilnehmerfunktion, new GridBagConstraints(1, 1, 1, 1, 0.1, 0.0,
				GridBagConstraints.CENTER, GridBagConstraints.BOTH, new Insets(2, 2, 2, 2), 0, 0));

		jPanelWorkingOn.add(wsfKostenstelle.getWrapperButton(), new GridBagConstraints(0, 2, 1, 1, 0.0, 0.0,
				GridBagConstraints.CENTER, GridBagConstraints.BOTH, new Insets(2, 2, 2, 2), 0, 0));
		jPanelWorkingOn.add(wsfKostenstelle.getWrapperTextField(), new GridBagConstraints(1, 2, 1, 1, 0.1, 0.0,
				GridBagConstraints.CENTER, GridBagConstraints.BOTH, new Insets(2, 2, 2, 2), 0, 0));

		jPanelWorkingOn.add(wlaAngelegt, new GridBagConstraints(0, 3, 1, 1, 0.0, 0.0, GridBagConstraints.CENTER,
				GridBagConstraints.BOTH, new Insets(2, 2, 2, 2), 0, 0));
		jPanelWorkingOn.add(wtfAngelegt, new GridBagConstraints(1, 3, 1, 1, 0.1, 0.0, GridBagConstraints.CENTER,
				GridBagConstraints.BOTH, new Insets(2, 2, 2, 2), 0, 0));
		jPanelWorkingOn.add(wcbExternerTeilnehmer, new GridBagConstraints(1, 4, 3, 1, 0.0, 0.0,
				GridBagConstraints.CENTER, GridBagConstraints.BOTH, new Insets(2, 0, 2, 2), 0, 0));
	}
	...
	private void setDefaults() throws Throwable {
		...
		// den Benutzer setzen
		Integer pkPersonal = LPMain.getTheClient().getIDPersonal();
		personalbenutzerDto = DelegateFactory.getInstance().getPersonalDelegate().personalFindByPrimaryKey(pkPersonal);
	...
	protected void eventItemchanged(EventObject eI) throws Throwable {
		...
		if (e.getID() == ItemChangedEvent.GOTO_DETAIL_PANEL) {
			if (e.getSource() == panelQueryFLRPersonal) {
				onAuftragTeilnehmerPersonalSelected(e);
			} else if (e.getSource() == panelQueryFLRPartner) {
				onAuftragTeilnehmerPartnerSelected(e);
			} else if (e.getSource() == panelQueryFLRTeilnehmerfunktion) {
				Integer iIdFunktion = (Integer) ((ISourceEvent) e.getSource()).getIdSelected();

				funktionDto = DelegateFactory.getInstance().getLocaleDelegate().funktionFindByPrimaryKey(iIdFunktion);

				wtfTeilnehmerfunktion.setText(funktionDto.getCNr());
			}
			/*
			 * else { // source ist die flr liste auf meinem panel Integer pkTeilnehmer =
			 * (Integer) ( (ISourceEvent) e.getSource()). getIdSelected(); if (pkTeilnehmer
			 * != null) { auftragteilnehmerDto =
			 * DelegateFactory.getInstance().getAuftragteilnehmerDelegate().
			 * auftragteilnehmerFindByPrimaryKey(pkTeilnehmer); dto2Components(); } }
			 */
		}
	}

	private void onAuftragTeilnehmerPersonalSelected(ItemChangedEvent e) throws Throwable {
		...
		if (DelegateFactory.getInstance().getAuftragteilnehmerDelegate()
				.istPartnerEinAuftragteilnehmer(personalDto.getPartnerIId(), tpAuftrag.getAuftragDto().getIId())) {
			DialogFactory.showModalDialog(LPMain.getTextRespectUISPr("lp.warning"),
					LPMain.getTextRespectUISPr("auft.teilnehmer.warning.personalbereitszugeordnet"));
		} else {
			// partnerteilnehmerDto = personalDto.getPartnerDto();
			auftragteilnehmerDto.setPartnerIIdAuftragteilnehmer(personalDto.getPartnerDto().getIId());

			wtfTeilnehmer.setText(personalDto.getPartnerDto().formatAnrede());
		}
	}

	private void onAuftragTeilnehmerPartnerSelected(ItemChangedEvent e) throws ExceptionLP {
		Integer iIdPartner = (Integer) ((ISourceEvent) e.getSource()).getIdSelected();

		PartnerDto partnerDto = DelegateFactory.getInstance().getPartnerDelegate().partnerFindByPrimaryKey(iIdPartner);

		if (DelegateFactory.getInstance().getAuftragteilnehmerDelegate().istPartnerEinAuftragteilnehmer(iIdPartner,
				tpAuftrag.getAuftragDto().getIId())) {
			DialogFactory.showModalDialog(LPMain.getTextRespectUISPr("lp.warning"),
					LPMain.getTextRespectUISPr("auft.teilnehmer.warning.partnerbereitszugeordnet"));
		} else {
			...
			try {
				...
				if (auftragteilnehmerDto.getIId() == null) {
					if (bFuegeNeuePositionVorDerSelektiertenEin) {
						Integer iIdAktuellePosition = (Integer) tpAuftrag.getAuftragTeilnehmerTop().getSelectedId();

						// die erste Position steht an der Stelle 1
						Integer iSortAktuellePosition = new Integer(1);

						if (iIdAktuellePosition != null) {
							iSortAktuellePosition = DelegateFactory.getInstance().getAuftragteilnehmerDelegate()
					...
					Integer pkTeilnehmer = DelegateFactory.getInstance().getAuftragteilnehmerDelegate()
					...
					// @todo in der Folge wird am Server fuer den entsprechenden Teilnehmer eine
					// Aufgabe erzeugt PJ 4818
				} else {
					DelegateFactory.getInstance().getAuftragteilnehmerDelegate()
							.updateAuftragteilnehmer(this.auftragteilnehmerDto);
				}
	...
	//PJ21112
	private boolean istAktualisierenTeilnehmerErlaubt() {
		if (tpAuftrag.getAuftragDto().getStatusCNr().equals(AuftragServiceFac.AUFTRAGSTATUS_ERLEDIGT) || tpAuftrag.getAuftragDto().getStatusCNr().equals(AuftragServiceFac.AUFTRAGSTATUS_STORNIERT)) {
			// SP6288
			DialogFactory.showModalDialog(LPMain.getTextRespectUISPr("lp.error"),
					LPMain.getTextRespectUISPr("auft.teilnehmer.aendern.error"));
			return false;
		} else {
			return true;
		}
	...
	public void eventActionNew(EventObject eventObject, boolean bLockMeI, boolean bNeedNoNewI) throws Throwable {
		if (istAktualisierenTeilnehmerErlaubt()) {
			...
			if (((ItemChangedEvent) eventObject).getID() == ItemChangedEvent.ACTION_MY_OWN_NEW) {
				// pqaddbutton: 5 Dieses Flag gibt an, ob aus Personal oder Partner gewaehlt
				// wird
			...
			tpAuftrag.getAuftragTeilnehmerTop()
					.updateButtons(tpAuftrag.getAuftragTeilnehmerBottom().getLockedstateDetailMainKey());
		}
	}
	...
	protected void eventActionDelete(ActionEvent e, boolean bAdministrateLockKeyI, boolean bNeedNoDeleteI)
			throws Throwable {
		if (istAktualisierenTeilnehmerErlaubt()) {
			DelegateFactory.getInstance().getAuftragteilnehmerDelegate().removeAuftragteilnehmer(auftragteilnehmerDto);
	...
	protected void eventActionSpecial(ActionEvent e) throws Throwable {
		...
		} else if (e.getActionCommand().equals(ACTION_SPECIAL_TEILNEHMERFUNKTION)) {
			dialogQueryTeilnehmerfunktion(e);
		}
	}

	/**
	 * Dialogfenster zur Auswahl eines Partners. <br>
	 * Erfasst werden externe Teilnehmer zu einem Auftrag
	 *
	 * @param e ActionEvent
	 * @throws java.lang.Throwable Ausnahme
	 */
	void dialogQueryPartner(ActionEvent e) throws Throwable {
	...
	void dialogQueryPersonal(ActionEvent e) throws Throwable {
		Integer iIdPartner = auftragteilnehmerDto.getPartnerIIdAuftragteilnehmer();

		if (iIdPartner != null) {
			iIdPartner = DelegateFactory.getInstance().getPersonalDelegate().personalFindByPartnerIIdMandantCNr(
					auftragteilnehmerDto.getPartnerIIdAuftragteilnehmer(), LPMain.getTheClient().getMandant()).getIId();
		}
	 ...
	 * @throws java.lang.Throwable Ausnahme
	 */
	void dialogQueryTeilnehmerfunktion(ActionEvent e) throws Throwable {
		QueryType[] qt = null;
		FilterKriterium[] fk = null;

		panelQueryFLRTeilnehmerfunktion = new PanelQueryFLR(qt, fk, QueryParameters.UC_ID_FUNKTION, null, intFrame,
				LPMain.getTextRespectUISPr("lp.funktionauswahlliste"), funktionDto.getIId());
	...
	private void dto2Components() throws Throwable {
		...
		wtfTeilnehmer.setText(DelegateFactory.getInstance().getPartnerDelegate()
				.partnerFindByPrimaryKey(auftragteilnehmerDto.getPartnerIIdAuftragteilnehmer()).formatAnrede());
		...
		wsfKostenstelle.setKey(auftragteilnehmerDto.getKostenstelleIId());
		...
		personalbenutzerDto = DelegateFactory.getInstance().getPersonalDelegate()
				.personalFindByPrimaryKey(pkPartnerAnlegen);
		wtfAngelegt.setText(personalbenutzerDto.getPartnerDto().formatAnrede());
		wcbExternerTeilnehmer.setSelected(Helper.short2boolean(auftragteilnehmerDto.getBIstExternerTeilnehmer()));
	}
	...
	private void components2Dto() throws ExceptionLP {
		...
		this.auftragteilnehmerDto.setKostenstelleIId(wsfKostenstelle.getIKey());

		// this.auftragteilnehmerDto.setPartnerIIdAuftragteilnehmer(
		// partnerteilnehmerDto.getIId());
		this.auftragteilnehmerDto.setAuftragteilnehmerfunktionIId(funktionDto.getIId());
