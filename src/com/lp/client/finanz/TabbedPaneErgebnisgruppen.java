...
/**
 * <p>
 * <I>Diese Klasse kuemmert sich um die Ergebnisgruppen</I>
 * </p>
 * <p>
 * Copyright Logistik Pur Software GmbH (c) 2004-2008
 * </p>
 * <p>
 * Erstellungsdatum <I>13.01.05</I>
 * </p>
 * <p>
 * </p>
 * 
 ...
 */
public class TabbedPaneErgebnisgruppen extends TabbedPane {
	...
	private static final String ACTION_SPECIAL_MEHRERE_LOESCHEN = PanelBasis.ALWAYSENABLED
			+ "action_special_mehrere_loeschen";
	private static final String ACTION_SPECIAL_MEHRERE_HINZUFUEGEN = PanelBasis.ALWAYSENABLED
			+ "action_special_mehrere_hinzufuegen";

	private PanelQueryFLR panelQueryFLRSachkonten = null;
	...
	void setErgebnisgruppeDto(ErgebnisgruppeDto ergebnisgruppeDto) throws Throwable {
		this.ergebnisgruppeDto = ergebnisgruppeDto;
		if (getErgebnisgruppeDto() != null) {
			getInternalFrame().setLpTitle(InternalFrame.TITLE_IDX_AS_I_LIKE, getErgebnisgruppeDto().getCBez());
		}
	...
	private void jbInit() throws Throwable {
		...
		if (bBilanzgruppe) {
			insertTab(LPMain.getInstance().getTextRespectUISPr("finanz.tab.unten.bilanzgruppen.title"), null, null,
					LPMain.getInstance().getTextRespectUISPr("finanz.tab.unten.bilanzgruppen.title"),
					IDX_ERGEBNISGRUPPEN);
		} else {
			insertTab(LPMain.getInstance().getTextRespectUISPr("finanz.ergebnisgruppen"), null, null,
					LPMain.getInstance().getTextRespectUISPr("finanz.ergebnisgruppen"), IDX_ERGEBNISGRUPPEN);
		}

		// Tab 2: Kopfdaten
		insertTab(LPMain.getInstance().getTextRespectUISPr("finanz.konten"), null, null,
				LPMain.getInstance().getTextRespectUISPr("finanz.konten"), IDX_KONTEN);

		// Default
		this.setSelectedComponent(getPanelSplitErgebnisgruppen(true));
		// damit gleich eine selektiert ist
		ItemChangedEvent it = new ItemChangedEvent(getPanelQueryErgebnisgruppen(true), ItemChangedEvent.ITEM_CHANGED);
	...
	public void lPEventItemChanged(ItemChangedEvent e) throws Throwable {
		if (e.getID() == ItemChangedEvent.GOTO_DETAIL_PANEL) {
			...
			} else if (e.getSource() == panelQueryFLRSachkonten) {

				Object[] keys = panelQueryFLRSachkonten.getSelectedIds();

				for (int i = 0; i < keys.length; i++) {
					Integer kontoIId = (Integer) keys[i];
					...
					DelegateFactory.getInstance().getFinanzDelegate().updateKonto(kontoDto);

				}

				if (panelQueryFLRSachkonten.getDialog() != null) {
					panelQueryFLRSachkonten.getDialog().setVisible(false);
				}
			...
			if (e.getSource() == getPanelQueryErgebnisgruppen(false)) {
				...
				if (iPos > 0) {
					Integer iIdPosition = (Integer) getPanelQueryErgebnisgruppen(true).getSelectedId();
					Integer iIdPositionMinus1 = (Integer) getPanelQueryErgebnisgruppen(true).getTable()
							.getValueAt(iPos - 1, 0);
					DelegateFactory.getInstance().getFinanzDelegate().vertauscheErgebnisgruppen(iIdPosition,
							iIdPositionMinus1);
			...
			if (e.getSource() == getPanelQueryErgebnisgruppen(false)) {
				...
				if (iPos < getPanelQueryErgebnisgruppen(true).getTable().getRowCount() - 1) {
					Integer iIdPosition = (Integer) getPanelQueryErgebnisgruppen(true).getSelectedId();
					Integer iIdPositionPlus1 = (Integer) getPanelQueryErgebnisgruppen(true).getTable()
							.getValueAt(iPos + 1, 0);
					DelegateFactory.getInstance().getFinanzDelegate().vertauscheErgebnisgruppen(iIdPosition,
							iIdPositionPlus1); // die Liste neu anzeigen und den richtigen Datensatz markieren
					getPanelQueryErgebnisgruppen(true).setSelectedId(iIdPosition);
				}
			}
		} else if (e.getID() == ItemChangedEvent.ACTION_POSITION_VORPOSITIONEINFUEGEN) {
			if (e.getSource() == getPanelQueryErgebnisgruppen(true)) {

				getPanelDetailErgebnisgruppeKopfdaten(true).eventActionNew(e, true, false);
				getPanelDetailErgebnisgruppeKopfdaten(true).eventYouAreSelected(false);
			}
		} else if (e.getID() == ItemChangedEvent.ACTION_SPECIAL_BUTTON) {
			String sAspectInfo = ((ISourceEvent) e.getSource()).getAspect();

			if (sAspectInfo.equals(ACTION_SPECIAL_MEHRERE_LOESCHEN)) {

				if (panelQueryKonten.getSelectedIds() != null && panelQueryKonten.getSelectedIds().length > 0) {
					for (int i = 0; i < panelQueryKonten.getSelectedIds().length; i++) {

						KontoDto kontoDto = DelegateFactory.getInstance().getFinanzDelegate()
								.kontoFindByPrimaryKey((Integer) panelQueryKonten.getSelectedIds()[i]);
			...
			} else if (sAspectInfo.equals(ACTION_SPECIAL_MEHRERE_HINZUFUEGEN)) {

				if (e.getSource() == getPanelQueryKonten(false)) {

					List<FilterKriterium> filters = new ArrayList<FilterKriterium>();
					filters.add(new FilterKriterium(FinanzFac.FLR_KONTO_KONTOTYP_C_NR, true,
							"'" + FinanzServiceFac.KONTOTYP_SACHKONTO + "'", FilterKriterium.OPERATOR_EQUAL, false));
					filters.add(new FilterKriterium(FinanzFac.FLR_KONTO_MANDANT_C_NR, true,
							"'" + LPMain.getTheClient().getMandant() + "'", FilterKriterium.OPERATOR_EQUAL, false));

					filters.add(FinanzFilterFactory.getInstance().createFKKontenOhneMitlaufende());

					filters.add(new FilterKriterium(FinanzFac.FLR_KONTO_ERGEBNISGRUPPE_I_ID, true, "",
					...
					panelQueryFLRSachkonten = new PanelQueryFLR(null, filters.toArray(new FilterKriterium[0]),
							QueryParameters.UC_ID_FINANZKONTEN, null, getInternalFrame(),
							LPMain.getTextRespectUISPr("finanz.liste.konten"), fkVersteckt, null);
					panelQueryFLRSachkonten.setMultipleRowSelectionEnabled(true);
					panelQueryFLRSachkonten.addButtonAuswahlBestaetigen(null);
					...
					panelQueryFLRSachkonten.befuellePanelFilterkriterienDirekt(fkDirekt1, fkDirekt2);

					new DialogQuery(panelQueryFLRSachkonten);
				}
			}
		}
	}
	 ...
	 * @throws Throwable
	 */
	private void holeErgebnisgruppeDto(Object key) throws Throwable {
		if (key != null) {
	 ...
	 * @throws Throwable
	 */
	private PanelFinanzErgebnisgruppeKopfdaten getPanelDetailErgebnisgruppeKopfdaten(boolean bNeedInstantiationIfNull)
			throws Throwable {
		if (panelFinanzErgebnisgruppeKopfdaten == null && bNeedInstantiationIfNull) {
			panelFinanzErgebnisgruppeKopfdaten = new PanelFinanzErgebnisgruppeKopfdaten(getInternalFrame(),
					LPMain.getInstance().getTextRespectUISPr("lp.kopfdaten"), this);
		}
		return panelFinanzErgebnisgruppeKopfdaten;
	}

	public PanelQuery getPanelQueryErgebnisgruppen(boolean bNeedInstantiationIfNull) throws Throwable {
		if (panelQueryErgebnisgruppen == null && bNeedInstantiationIfNull) {
			String[] aWhichButtonIUseErgebnisgruppen = { PanelBasis.ACTION_NEW,
			...
			panelQueryErgebnisgruppen = new PanelQuery(qtErgebnisgruppen,
					FinanzFilterFactory.getInstance().createFKErgebnisgruppe(bBilanzgruppe),
					QueryParameters.UC_ID_ERGEBNISGRUPPE, aWhichButtonIUseErgebnisgruppen, getInternalFrame(),
					LPMain.getInstance().getTextRespectUISPr("lp.auswahl"), true);
		}
		return panelQueryErgebnisgruppen;
	}

	private PanelSplit getPanelSplitErgebnisgruppen(boolean bNeedInstantiationIfNull) throws Throwable {
		if (panelSplitErgebnisgruppen == null && bNeedInstantiationIfNull) {
			panelSplitErgebnisgruppen = new PanelSplit(getInternalFrame(), getPanelDetailErgebnisgruppeKopfdaten(true),
	 ...
	 * @throws Throwable
	 */
	private PanelQuery getPanelQueryKonten(boolean bNeedInstantiationIfNull) throws Throwable {
		if (panelQueryKonten == null && bNeedInstantiationIfNull) {
			...
			panelQueryKonten = new PanelQuery(qtKonten, filtersKonten, QueryParameters.UC_ID_FINANZKONTEN,
			...
			panelQueryKonten.setMultipleRowSelectionEnabled(true);

			panelQueryKonten.createAndSaveAndShowButton("/com/lp/client/res/document.png",
					LPMain.getInstance().getTextRespectUISPr("lp.new"), ACTION_SPECIAL_MEHRERE_HINZUFUEGEN,
					KeyStroke.getKeyStroke('N', java.awt.event.InputEvent.CTRL_MASK), RechteFac.RECHT_FB_FINANZ_CUD);

			panelQueryKonten.createAndSaveAndShowButton("/com/lp/client/res/delete2.png",
					LPMain.getInstance().getTextRespectUISPr("lp.delete"), ACTION_SPECIAL_MEHRERE_LOESCHEN,
					KeyStroke.getKeyStroke('D', java.awt.event.InputEvent.CTRL_MASK), RechteFac.RECHT_FB_FINANZ_CUD);
	...
	private void refreshFiltersKonten() throws Throwable {
