...
public class PanelCockpitAnsprechpartner extends PanelAnsprechpartner {
	/**
	 *
	 */
	private static final long serialVersionUID = 1L;

	public PanelCockpitAnsprechpartner(InternalFrame internalFrame,
			String add2TitleI, Object keyI) throws Throwable {
		super(internalFrame, add2TitleI, keyI);
		zeilenFuerCockpitAusblenden();
	}
	...
	protected void setDefaults(Integer iSort) throws Throwable {
		...
		getAnsprechpartnerDto().setPartnerIId(
				getInternalFrameCockpit().getTpCockpit().getServicePartnerDto()
						.getIId());

		if (iSort == null) {
			// eine neue Position bekommt ein eindeutiges iSort
			iSort = DelegateFactory
					.getInstance()
					.getAnsprechpartnerDelegate()
					.getMaxISort(
							getInternalFrameCockpit().getTpCockpit()
									.getServicePartnerDto().getIId());
			iSort = new Integer(iSort.intValue()
					+ AnsprechpartnerISortValues.ANSPRECHPARTNER_ISORT_STEP);
		} else {
			iSort = new Integer(iSort.intValue()
					- AnsprechpartnerISortValues.ANSPRECHPARTNER_ISORT_STEP / 2);
		}
	...
	public AnsprechpartnerDto getAnsprechpartnerDto() {
		return getInternalFrameCockpit().getTpCockpit().getAnsprechpartnerDto();
	}

	protected void setAnsprechpartnerDto(AnsprechpartnerDto ansprechpartnerDtoI) {
		getInternalFrameCockpit().getTpCockpit().setAnsprechpartnerDto(
				ansprechpartnerDtoI);
	}
	...
	protected String getSelectedPartnerTitelAnrede() {
		String t1 = getInternalFrameCockpit().getTpCockpit().getServicePartnerDto()
	...
	protected PartnerDto getPartnerDtoPartner() {
		return getInternalFrameCockpit().
				getTpCockpit().getServicePartnerDto();
	}
}
