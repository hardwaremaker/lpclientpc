...
public class PanelIdentifikation extends PanelBasis {
	...
	private IdentifikationDto identifikationDto = null;
	private WrapperLabel wlaBezeichnung = new WrapperLabel();
	private WrapperTextField wtfBezeichnung = new WrapperTextField();

	public PanelIdentifikation(InternalFrame internalFrame, String add2TitleI,
	...
	public void eventActionNew(EventObject eventObject, boolean bLockMeI,
		...
		identifikationDto = new IdentifikationDto();
	...
	protected void eventActionDelete(ActionEvent e,
		...
		DelegateFactory.getInstance().getPartnerServicesDelegate()
				.removeIdentifikation(identifikationDto);
		this.setKeyWhenDetailPanel(null);
		super.eventActionDelete(e, false, false);
	}

	protected void components2Dto() {
		identifikationDto.setCNr(wtfKennung.getText());
		if (identifikationDto.getIdentifikationsprDto() == null) {
			identifikationDto.setIdentifikationsprDto(new IdentifikationsprDto());
		}
		identifikationDto.getIdentifikationsprDto().setCBez(wtfBezeichnung.getText());

	}

	protected void dto2Components() {
		wtfKennung.setText(identifikationDto.getCNr());
		if (identifikationDto.getIdentifikationsprDto() != null) {
			wtfBezeichnung.setText(identifikationDto.getIdentifikationsprDto()
					.getCBez());
		} else {
	...
	public void eventActionSave(ActionEvent e, boolean bNeedNoSaveI)
		...
		if (allMandatoryFieldsSetDlg()) {
			components2Dto();
			if (identifikationDto.getIId() == null) {
				identifikationDto.setIId(DelegateFactory.getInstance()
						.getPartnerServicesDelegate()
						.createIdentifikation(identifikationDto));
				setKeyWhenDetailPanel(identifikationDto.getIId());
			} else {
				DelegateFactory.getInstance().getPartnerServicesDelegate()
						.updateIdentifikation(identifikationDto);
			}
			super.eventActionSave(e, true);

			if (getInternalFrame().getKeyWasForLockMe() == null) {
				getInternalFrame().setKeyWasForLockMe(identifikationDto.getCNr());
			}
	...
	protected String getLockMeWer() throws Exception {
		return HelperClient.LOCKME_IDENTIFIKATION;
	}
	...
	public void eventYouAreSelected(boolean bNeedNoYouAreSelectedI)
		...
		if (key == null || (key.equals(LPMain.getLockMeForNew()))) {
			...
			identifikationDto = DelegateFactory.getInstance()
					.getPartnerServicesDelegate()
