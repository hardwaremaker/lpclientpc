 ...
 * @author werner
 */
public class QueryInputRow extends JPanel {
	...
	/**
	 * the width of the text field where the user can type the criteria's value in.
	 */
	...
	private WrapperDateField dateFieldValue;
	...
	/**
	 * determines wether the value from the user input should be wrapped in single
	 * quotes (example: 'value' instead of value) before setting it in
	 ...
	 * (example: %value% instead of value) before setting it in FilterKriterium. If
	 * true wrapping is enabled.
	 */
	private boolean wrapValueWithProzent = false;

	/**
	 * determines wheter the equal operator should be replaced by the like operator
	 * when set in the FilterKriterium. This is useful if "=" should be displayed in
	 * the operator combobox, but like is required in the query, as it is for
	 * Strings and Dates.
	 */
	...
	public QueryInputRow(QueryType type, PanelBasis lPPanelI) {
		...
		this.kriterium = new FilterKriterium(tmpKriterium.kritName, tmpKriterium.isKrit, tmpKriterium.value,
				tmpKriterium.operator, tmpKriterium.isBIgnoreCase(), tmpKriterium.iTyp);
...
//        QueryInputRow.HEIGHT));
		...
		if (this.replaceEqualByLike && FilterKriterium.OPERATOR_LIKE.equals(this.kriterium.operator)) {
			this.comboBoxLogicalOperator.setSelectedItem(FilterKriterium.OPERATOR_EQUAL);
		} else {
			this.comboBoxLogicalOperator.setSelectedItem(this.kriterium.operator);
		}

		// texteingabe
		this.textFieldValue = new WrapperTextField(this.kriterium.value, WIDTH_TEXT_FIELD_VALUE);
		// texteingabe
		this.dateFieldValue = new WrapperDateField();

		// so mache ich schnell einen button.
		buttonRemoveFilterzeile = lPPanel.createButtonActionListenerThis("/com/lp/client/res/minus_sign.png",
				"Entferne diese Filterzeile.", PanelQuery.ACTION_REMOVE_FILTERKRITERIUM);
		buttonRemoveFilterzeile.setPreferredSize(
		...
		comboBoxLogicalOperator.setMinimumSize(new Dimension(Defaults.sizeFactor(50), 23));
		comboBoxLogicalOperator.setPreferredSize(new Dimension(Defaults.sizeFactor(50), 23));
		// textFieldValue.setMinimumSize(new Dimension((int)d3.getSize().getWidth(),
		// 23));
		// textFieldValue.setPreferredSize(new Dimension((int)d3.getSize().getWidth(),
		// 23));
		...
		buttonRemoveFilterzeile.setMinimumSize(
				new Dimension((int) d4.getSize().getWidth(), Defaults.getInstance().getControlHeight()));
		buttonRemoveFilterzeile.setPreferredSize(
				new Dimension((int) d4.getSize().getWidth(), Defaults.getInstance().getControlHeight()));
		this.add(this.labelRowName);
		this.add(this.comboBoxLogicalOperator);
		if (tmpKriterium.iTyp == FilterKriterium.TYP_DATE) {
			this.add(this.dateFieldValue);
		} else {
...
//    SwingUtilities.getRootPane(this).repaint();
	...
	protected void fireRemoveButtonPressed(QueryInputRowEvent e) {
		for (int i = 0; i < this.listeners.size(); i++) {
			QueryInputRowListener listener = (QueryInputRowListener) this.listeners.elementAt(i);
			listener.removeButtonPressed(e);
		}
	}

	/**
	 * builds the FilterKriterium for use in a query according to the user's input.
	 * If isWrapValueWithSingleQuotes() is true, the entered text will be wrapped in
	 * single quotes before setting it in the return value. If the selected operator
	 * is the equal operator and replaceEqualByLike is true, then the
	 * FilterKriterium will contain the like operator instead.
	 *
	...
	public FilterKriterium getKriterium() {
		...
		if (this.kriterium.iTyp == FilterKriterium.TYP_DATE) {
			if (dateFieldValue.getDate() != null) {
				textValue = Helper.formatDateWithSlashes(dateFieldValue.getDate());
			}
		} else {
			textValue = textFieldValue.getText();
		}

		if (textValue != null && textValue.length() > 0) {
			if (this.wrapValueWithProzent) {
				if (this.kriterium.iTyp == FilterKriterium.TYP_STRING) {
					textValue = "%" + textValue + "%";
				}
			}
			if (this.wrapValueWithSingleQuotes || this.kriterium.iTyp == FilterKriterium.TYP_DATE) {
				textValue = "'" + textValue + "'";
			}
			this.kriterium.value = textValue;
			this.kriterium.isKrit = true;
			String operator = (String) this.comboBoxLogicalOperator.getSelectedItem();
			if (operator.equals(FilterKriterium.OPERATOR_EQUAL) && this.replaceEqualByLike) {
				operator = FilterKriterium.OPERATOR_LIKE;
			}
			if (operator.equals(FilterKriterium.OPERATOR_NOT_EQUAL) && this.replaceEqualByLike) {
				operator = FilterKriterium.OPERATOR_NOT_LIKE;
			}
	...
	/**
	 * @param wrapValueWithSingleQuotes The wrapValueWithSingleQuotes to set.
	 */
