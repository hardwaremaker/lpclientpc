...
public class ReportMaschinenzeitdaten extends PanelBasis implements PanelReportIfJRDS {
	/**
	 * 
	 */
	...
	private WrapperCheckBox wcbAlleMaschinen = new WrapperCheckBox();

	private Integer maschineIId = null;
	private WrapperDateRangeController wdrBereich = null;

	private boolean bErstelleReportSofort = false;

	public ReportMaschinenzeitdaten(InternalFrameZeiterfassung internalFrame, String add2Title) throws Throwable {
	...
	public ReportMaschinenzeitdaten(InternalFrameZeiterfassung internalFrame, String add2Title, boolean bBisHeute)
			throws Throwable {
		super(internalFrame, add2Title);
		LPMain.getInstance();
		LPMain.getTextRespectUISPr("zeiterfassung.report.maschinenzeitdaten");
	...
	public JasperPrintLP getReport(String sDrucktype) throws Throwable {
		...
		java.sql.Timestamp wdfBisTemp = Helper
				.addiereTageZuTimestamp(new java.sql.Timestamp(wdfBis.getTimestamp().getTime()), 1);

		return DelegateFactory.getInstance().getZeiterfassungReportDelegate().printMaschinenzeitdaten(maschineIId,
				wdfVon.getTimestamp(), wdfBisTemp, wcbAlleMaschinen.isSelected());
	}
	...
	private void jbInit() throws Throwable {
		...
		wbuPersonal.setText(
				LPMain.getTextRespectUISPr("zeiterfassung.report.maschinenzeitdaten.selektiertemaschine") + ": ");
		...
		wcbAlleMaschinen.setText(LPMain.getTextRespectUISPr("pers.maschinenzeitdaten.allemaschinen"));
		...
		jpaWorkingOn.add(wbuGestern, new GridBagConstraints(6, 5, 1, 1, 0.0, 0.0, GridBagConstraints.WEST,
				GridBagConstraints.BOTH, new Insets(2, 2, 2, 2), -15, 0));

		jpaWorkingOn.add(wlaZeitraum, new GridBagConstraints(0, 4, 1, 1, 0.0, 0.0, GridBagConstraints.CENTER,
				GridBagConstraints.HORIZONTAL, new Insets(2, 2, 2, 2), 0, 0));
		jpaWorkingOn.add(wtfPersonal, new GridBagConstraints(2, 2, 3, 1, 0.0, 0.0, GridBagConstraints.CENTER,
				GridBagConstraints.HORIZONTAL, new Insets(2, 2, 2, 2), 0, 0));

		jpaWorkingOn.add(wcbAlleMaschinen, new GridBagConstraints(2, 3, 3, 1, 0.0, 0.0, GridBagConstraints.CENTER,
				GridBagConstraints.HORIZONTAL, new Insets(2, 2, 2, 2), 0, 0));

		jpaWorkingOn.add(wbuPersonal, new GridBagConstraints(0, 2, 1, 1, 0.0, 0.0, GridBagConstraints.CENTER,
				GridBagConstraints.HORIZONTAL, new Insets(2, 2, 2, 2), 0, 0));
	}
