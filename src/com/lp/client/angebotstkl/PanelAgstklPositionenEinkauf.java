...
public class PanelAgstklPositionenEinkauf extends PanelPositionen2 {
	...
	public PanelAgstklPositionenEinkauf(InternalFrame internalFrame, String add2TitleI, Object key) throws Throwable {
		super(internalFrame, add2TitleI, key, PanelPositionen2.TYP_PANELPREISEINGABE_ARTIKELEINKAUF);
	...
	private void jbInit() throws Throwable {
		...
		bVkPreisAusStklGesamtkalkulationWennAgstkl = ((Boolean) parametermandantDto.getCWertAsObject()).booleanValue();
		...
		add(panelPosition, new GridBagConstraints(0, 7, 1, 1, 1, 0, GridBagConstraints.CENTER,
				GridBagConstraints.HORIZONTAL, new Insets(2, 2, 2, 2), 0, 0));

		/*
		 * add(wcbMitdrucken, new GridBagConstraints(0, 7, 1, 1, 1.0, 0.0,
		 * GridBagConstraints.EAST, GridBagConstraints.HORIZONTAL, new Insets(0, 220,
		 * 10, 0), 0, 0));
		 */
		...
		if (intFrame.getAgstklDto() != null && intFrame.getAgstklDto().getIId() != null) {
			...
			if (default_dAufschlag.doubleValue() != -1) {
				...
				if (panelArtikel.wnfAufschlagProzent != null) {
					panelArtikel.wnfAufschlagProzent.setDouble(default_dAufschlag);
				}
				if (panelHandeingabe.wnfAufschlagProzent != null) {
					panelHandeingabe.wnfAufschlagProzent.setDouble(default_dAufschlag);
				}
			}
	...
	protected void eventActionUpdate(ActionEvent aE, boolean bNeedNoUpdateI) throws Throwable {
		...
		((PanelPositionenArtikelEinkauf) panelArtikel).wbuPreisauswahl.setEnabled(true);
	...
	protected void eventActionSpecial(ActionEvent e) throws Throwable {
		if (e.getActionCommand().equals(PanelPositionenArtikelEinkauf.ACTION_SPECIAL_EK_PREIS_HOLEN)) {
			try {
				setPreis();
	...
	protected void eventItemchanged(EventObject eI) throws Throwable {
		...
		if (e.getID() == ItemChangedEvent.GOTO_DETAIL_PANEL) {
			if (e.getSource() == ((PanelPositionenArtikelEinkauf) panelArtikel).wifArtikelauswahl) {
				if (((PanelPositionenArtikelEinkauf) panelArtikel).wifArtikelauswahl.getArtikelDto() != null) {
	...
	protected void setPreis() throws Throwable {
		...
		ArtikellieferantDto artLiefDto = DelegateFactory.getInstance().getArtikelDelegate().getArtikelEinkaufspreis(
				panelArtikel.getArtikelDto().getIId(), null, panelArtikel.wnfMenge.getBigDecimal(),
				tpAngebotstkl.getInternalFrameAngebotstkl().getAgstklDto().getWaehrungCNr(), new java.sql.Date(
						tpAngebotstkl.getInternalFrameAngebotstkl().getAgstklDto().getTBelegdatum().getTime()));

		if (bVkPreisAusStklGesamtkalkulationWennAgstkl) {

			if (!Helper.short2boolean(panelArtikel.getArtikelDto().getBKalkulatorisch())
					&& tpAngebotstkl.getInternalFrameAngebotstkl().getAgstklDto().getKundeIId() != null) {
				...
				KundeDto kundeDto = DelegateFactory.getInstance().getKundeDelegate().kundeFindByPrimaryKey(
						tpAngebotstkl.getInternalFrameAngebotstkl().getAgstklDto().getKundeIId());
				Integer kundeStandardpreisliste = kundeDto.getVkpfArtikelpreislisteIIdStdpreisliste();

				MwstsatzDto mwstsatzDtoAktuell = DelegateFactory.getInstance().getMandantDelegate().mwstsatzFindZuDatum(
						kundeDto.getMwstsatzbezIId(),
						tpAngebotstkl.getInternalFrameAngebotstkl().getAgstklDto().getTBelegdatum());
				...
				vkpreisfindungDto = DelegateFactory.getInstance().getVkPreisfindungDelegate().verkaufspreisfindung(
						...
						new java.sql.Date(
								tpAngebotstkl.getInternalFrameAngebotstkl().getAgstklDto().getTBelegdatum().getTime()),
						kundeStandardpreisliste, mwstsatzDtoAktuell.getIId(),
						tpAngebotstkl.getInternalFrameAngebotstkl().getAgstklDto().getWaehrungCNr(), true);

				VerkaufspreisDto verkaufspreisDtoInZielwaehrung = Helper.getVkpreisBerechnet(vkpreisfindungDto);

				BigDecimal verkaufspreis = null;

				BigDecimal preisausGesamtkalkulation = null;

				if (verkaufspreisDtoInZielwaehrung != null) {
					verkaufspreis = verkaufspreisDtoInZielwaehrung.nettopreis;
				}

				StuecklisteDto stklDto = DelegateFactory.getInstance().getStuecklisteDelegate()
						.stuecklisteFindByMandantCNrArtikelIIdOhneExc(panelArtikel.getArtikelDto().getIId());

				if (stklDto != null) {
					VerkaufspreisDto vkPreisDtoStkl = DelegateFactory.getInstance().getStuecklisteDelegate()
							.getKalkuliertenVerkaufspreisAusGesamtkalkulation(panelArtikel.getArtikelDto().getIId(),
									panelArtikel.wnfMenge.getBigDecimal(),
									new java.sql.Date(tpAngebotstkl.getInternalFrameAngebotstkl().getAgstklDto()
											.getTBelegdatum().getTime()),
									tpAngebotstkl.getInternalFrameAngebotstkl().getAgstklDto().getWaehrungCNr());

					if (vkPreisDtoStkl != null) {
						preisausGesamtkalkulation = vkPreisDtoStkl.nettopreis;
					}
				...
				if (verkaufspreis != null) {
					...
					artLiefDto.setNEinzelpreis(verkaufspreis);
					artLiefDto.setNNettopreis(verkaufspreis);
					...
					if (stklDto != null && Helper.short2boolean(stklDto.getBFremdfertigung())) {
						// Wenn es keinen Verkaufspreis gibt und die Stueckliste fremdgefertig ist, dann
						// gilt der Einkaufspreis
					} else if (stklDto != null && !Helper.short2boolean(stklDto.getBFremdfertigung())
							&& preisausGesamtkalkulation != null) {
						// Ist es eine Eigenfertigung dann gilt der Preis aus der Gesamtkalkulation
						artLiefDto = new ArtikellieferantDto();
						artLiefDto.setNEinzelpreis(preisausGesamtkalkulation);
						artLiefDto.setNNettopreis(preisausGesamtkalkulation);
						artLiefDto.setFRabatt(0D);
						artLiefDto.setBRabattbehalten(Helper.boolean2Short(false));

					} else if (stklDto != null && Helper.short2boolean(stklDto.getBFremdfertigung())
							&& artLiefDto == null && preisausGesamtkalkulation != null) {
						// Ist es eine Fremdfertigung und kein EK-PReis gefunden dann gilt der Preis aus
		...
		if (artLiefDto != null) {
			if (artLiefDto.getNMaterialzuschlag() != null) {
				panelArtikel.wnfMaterialzuschlag.setBigDecimal(artLiefDto.getNMaterialzuschlag());
			}
			...
			if (artLiefDto.getFRabatt() != null) {
				...
				if (artLiefDto.getNNettopreis() != null) {
					panelArtikel.wnfRabattsumme.setBigDecimal(
							Helper.getProzentWert(artLiefDto.getNEinzelpreis(), new BigDecimal(artLiefDto.getFRabatt()),
		...
		if (default_dAufschlag == -1) {
			...
			Timestamp belegDatum = intFrame.getAgstklDto().getTBelegdatum();
			if (belegDatum == null) {
				belegDatum = HelperTimestamp.cut();
			}
			MwstsatzDto mwstsatzDtoAktuell = DelegateFactory.getInstance().getMandantDelegate()
					.mwstsatzFindZuDatum(kdDto.getMwstsatzbezIId(), belegDatum);
			/*
			...
			VkpreisfindungDto vkpreisfindungDto = DelegateFactory.getInstance().getVkPreisfindungDelegate()
					.verkaufspreisfindung(panelArtikel.getArtikelDto().getIId(), kdDto.getIId(),
							panelArtikel.wnfMenge.getBigDecimal(),
							new java.sql.Date(intFrame.getAgstklDto().getTBelegdatum().getTime()),
			...
			if (vkPreisDto != null) {
				panelArtikel.wnfGesamtpreisMitAufschlag.setBigDecimal(vkPreisDto.nettopreis);
							...
							if (agstklpositionDto.getIId() == null) {
								bPositionFuerZugehoerigenArtikelAnlegen = DialogFactory
										.pruefeZugehoerigenArtikelAnlegenDlg(panelArtikel.getArtikelDto(),
												agstklpositionDto.getNMenge(), true, this);
							}
						}
					}
				}
	...
	private void dto2Components() throws Throwable {
		...
		super.dto2Components(agstklpositionDto, kdDto.getPartnerDto().getLocaleCNrKommunikation());
		...
		if (positionsart.equalsIgnoreCase(AngebotstklServiceFac.AGSTKLPOSITIONART_IDENT)) {
			...
			panelArtikel.getWnfRabattsatz().setDouble(agstklpositionDto.getFRabattsatz());
			panelArtikel.wnfEinzelpreis.setBigDecimal(agstklpositionDto.getNNettoeinzelpreis());
			panelArtikel.wnfRabattsumme.setBigDecimal(Helper.getProzentWert(agstklpositionDto.getNNettoeinzelpreis(),
					new BigDecimal(agstklpositionDto.getFRabattsatz()), 4));
			panelArtikel.wnfMaterialzuschlag.setBigDecimal(agstklpositionDto.getNMaterialzuschlag());
			panelArtikel.wnfNettopreis.setBigDecimal(agstklpositionDto.getNNettogesamtpreis());

			/*
			 * panelArtikel.setArtikelDto(agstklpositionDto); if (oArtikelDto.getIId() !=
			 * null) { artLiefDto = DelegateFactory .getInstance() .getArtikelDelegate()
			 * .artikellieferantFindByArtikellIIdLieferantIIdOhneExc( oArtikelDto.getIId(),
			...
			if (Helper.short2Boolean((agstklpositionDto.getBRabattsatzuebersteuert())) == true) {
				panelArtikel.wnfRabattsumme.getWrbFixNumber().setSelected(true);
			} else {
				panelArtikel.wnfNettopreis.getWrbFixNumber().setSelected(true);
			}
	...
	private void components2Dto() throws Throwable {
		...
		super.components2Dto(agstklpositionDto, kdDto.getPartnerDto().getLocaleCNrKommunikation(),
		...
		if (positionsart.equalsIgnoreCase(AngebotstklServiceFac.AGSTKLPOSITIONART_IDENT)) {
			...
			agstklpositionDto.setBRabattsatzuebersteuert(panelArtikel.wnfRabattsumme.getWrbFixNumber().getShort());
			agstklpositionDto.setBNettopreisuebersteuert(panelArtikel.wnfNettopreis.getWrbFixNumber().getShort());
