...
public class DialogKundesokoimportXLS extends JDialog implements ActionListener {
	...
	private TabbedPaneKunde tp=null;

	public DialogKundesokoimportXLS(TabbedPaneKunde tp, Integer kundeIId) throws Throwable {
		
			super(LPMain.getInstance().getDesktop(), "Kunden-SOKO importieren ",
					true);
		...
		HvOptional<XlsFile> xlsFile = FileOpenerFactory.kundeSokoImportXls(this);
		if (!xlsFile.isPresent()) return;
		
		xlsDatei = xlsFile.get().getBytes();
	...
	private void jbInit() throws Throwable {
		...
		wtaFehler.setText(DelegateFactory
				.getInstance()
				.getArtikelDelegate()
				.importiereKundesokoXLS(xlsDatei, kundeIId,
						false));
		...
		if (e.getSource().equals(wbuImportieren)) {
			try {
				DelegateFactory
						.getInstance()
						.getArtikelDelegate()
						.importiereKundesokoXLS(xlsDatei,
								kundeIId, true);
