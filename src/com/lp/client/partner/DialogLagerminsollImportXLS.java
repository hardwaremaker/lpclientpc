...
public class DialogLagerminsollImportXLS extends JDialog implements
		ActionListener {
	/**
	 * 
	 */
	...
	private TabbedPaneArtikel tp = null;

	public DialogLagerminsollImportXLS(TabbedPaneArtikel tp) throws Throwable {
		super(LPMain.getInstance().getDesktop(),
				"Lager Soll - Mindeststand importieren", true);
		...
		XlsFileOpener xlsOpener = FileOpenerFactory.lagerminsollImportOpenXls(tp.getInternalFrame());
	...
	private void jbInit() throws Throwable {
		...
		wtaFehler.setText(DelegateFactory.getInstance().getArtikelDelegate()
				.importiereLagerminSollXLS(xlsDatei, false));
		...
		if (e.getSource().equals(wbuImportieren)) {
			try {
				DelegateFactory.getInstance().getArtikelDelegate()
						.importiereLagerminSollXLS(xlsDatei, true);
