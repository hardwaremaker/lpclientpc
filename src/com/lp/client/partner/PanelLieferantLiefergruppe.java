 ...
 */
public class PanelLieferantLiefergruppe extends PanelBasis {
	/**
	 * 
	 */
	...
	static final public String ACTION_SPECIAL_FLR_LIEFERGRUPPE = "action_special_flr_liefergruppe";
	private PanelQueryFLR panelQueryFLRLiefergruppeAuswahl = null;
	private WrapperTextField wtfLiefergruppe = null;

	public PanelLieferantLiefergruppe(InternalFrame internalFrame, String add2TitleI, Object keyI) throws Throwable {
	...
	private void jbInit() throws Throwable {
		...
		String[] aButton = { PanelBasis.ACTION_SAVE, PanelBasis.ACTION_DELETE, PanelBasis.ACTION_DISCARD };
		...
		// Ab hier einhaengen.
		// Zeile
		jpaWorking.add(wbuLiefergruppe, new GridBagConstraints(0, iZeile, 1, 1, 0.2, 0.0, GridBagConstraints.CENTER,
				GridBagConstraints.BOTH, new Insets(2, 2, 2, 2), 0, 0));
		jpaWorking.add(wtfLiefergruppe, new GridBagConstraints(1, iZeile, 1, 1, 0.2, 0.0, GridBagConstraints.CENTER,
				GridBagConstraints.BOTH, new Insets(2, 2, 2, 2), 0, 0));
	}
	...
	public void eventYouAreSelected(boolean bNeedNoYouAreSelectedI) throws Throwable {
		...
		if (key == null || (key != null && key.equals(LPMain.getLockMeForNew()))) {
			...
			// Update.
			getInternalFrameLieferant().setLflfliefergruppeDto(DelegateFactory.getInstance().getLieferantDelegate()
					.lflfliefergruppeFindByPrimaryKey(oKey.getLieferant_i_id(), oKey.getLfliefergruppe_i_id()));
			dto2Components();
	...
	public void eventActionSave(ActionEvent e, boolean bNeedNoSaveI) throws Throwable {
		...
		if (allMandatoryFieldsSetDlg()) {
			...
			getInternalFrameLieferant().getLflfliefergruppeDto()
					.setLieferantIId(getInternalFrameLieferant().getLieferantDto().getIId());
			...
			FLRLFLiefergruppePK oKey = new FLRLFLiefergruppePK(lfpk.getLieferantIId(), lfpk.getLfliefergruppeIId());
	...
	protected void eventActionSpecial(ActionEvent e) throws Throwable {
		...
		if (e.getActionCommand().equals(ACTION_SPECIAL_FLR_LIEFERGRUPPE)) {
			...
			DialogQuery dialogQueryAnsprechpartner = new DialogQuery(panelQueryFLRLiefergruppeAuswahl);
		}
	}
	...
	protected void eventItemchanged(EventObject eI) throws ExceptionLP, Throwable {
		...
		if (e.getID() == ItemChangedEvent.GOTO_DETAIL_PANEL) {
			...
			if (e.getSource() == panelQueryFLRLiefergruppeAuswahl) {
				...
				getInternalFrameLieferant().getLflfliefergruppeDto().setLfliefergruppeIId(iId);

				LfliefergruppeDto lflfliefergruppeDto = null;
				if (iId != null) {
					lflfliefergruppeDto = DelegateFactory.getInstance().getLieferantServicesDelegate()
							.lfliefergruppeFindByPrimaryKey(iId);
					wtfLiefergruppe.setText(lflfliefergruppeDto != null ? lflfliefergruppeDto.getCNr() : null);
				}
			}
		}
	}
