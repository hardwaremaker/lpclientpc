...
/**
 * <p>
 * Diese Klasse kuemmert sich um das BankCRUD.
 * </p>
 *
 * <p>
 ...
 */
public class TabbedPaneBank extends TabbedPane implements IPartnerDtoService {
	/**
	 *
	 */
	private static final long serialVersionUID = 1L;
	public static int IDX_PANE_BANK_QP1 = 0;
	public static int IDX_PANE_BANK_D2 = 1;
	...
	public TabbedPaneBank(InternalFrame internalFrameI) throws Throwable {
	...
	private void jbInit() throws Throwable {
		...
		insertTab(LPMain.getInstance().getTextRespectUISPr("anf.panel.auswahl"), null, null,
				LPMain.getInstance().getTextRespectUISPr("anf.panel.auswahl"), IDX_PANE_BANK_QP1);

		// 2 tab oben; D2; lazy loading
		insertTab(LPMain.getInstance().getTextRespectUISPr("lp.detail"), null, null,
				LPMain.getInstance().getTextRespectUISPr("lp.detail"), IDX_PANE_BANK_D2);
		...
		getBankDto().setPartnerIId((Integer) panelBankQP1.getSelectedId());

		ItemChangedEvent it = new ItemChangedEvent(panelBankQP1, ItemChangedEvent.ITEM_CHANGED);
	...
	public void lPEventItemChanged(ItemChangedEvent eI) throws Throwable {
		...
		if (eI.getID() == ItemChangedEvent.ITEM_CHANGED) {
			if (eI.getSource() == panelBankQP1) {
				...
				refreshBankD2((Integer) panelBankQP1.getSelectedId());
				panelBankD2.setKeyWhenDetailPanel(panelBankQP1.getSelectedId());
	...
	protected void lPActionEvent(java.awt.event.ActionEvent e)throws Throwable {
		if (e.getActionCommand().equals(MENU_INFO_AENDERUNGEN)) {
			...
			getInternalFrame().showReportKriterien(new ReportEntitylog(HvDtoLogClass.BANK,
					getBankDto().getIId() + "", getInternalFrame(), add2Title,
					getBankDto().getPartnerDto().formatFixName1Name2()));
		} 
	}

	private void refreshBankD2(Integer iIdI) throws Throwable {
		if (panelBankD2 == null) {
			panelBankD2 = new PanelBank(getInternalFrame(), LPMain.getInstance().getTextRespectUISPr("lp.detail"), iIdI,
					this, this);
			setComponentAt(IDX_PANE_BANK_D2, panelBankD2);
		}
	}

	private void refreshBankQP1() throws Throwable {
		if (panelBankQP1 == null) {
			...
			panelBankQP1 = new PanelQuery(querytypes, filters, QueryParameters.UC_ID_BANK, aWhichStandardButtonIUse,
					getInternalFrame(), LPMain.getInstance().getTextRespectUISPr("anf.panel.auswahl"), true);

			setComponentAt(IDX_PANE_BANK_QP1, panelBankQP1);

			// Hier den zusaetzlichen Button aufs Panel bringen
			panelBankQP1.createAndSaveAndShowButton("/com/lp/client/res/businessmen16x16.png",
					LPMain.getInstance().getTextRespectUISPr("part.tooltip.new_bank_from_partner"),
					PanelBasis.ACTION_MY_OWN_NEW + EXTRA_NEU_AUS_PARTNER, null);

			panelBankQP1.befuellePanelFilterkriterienDirekt(PartnerFilterFactory.getInstance().createFKDBankName(),
	...
	public PartnerDto getServicePartnerDto() {
		return getBankDto().getPartnerDto();
	}

	@Override
	public void setServicePartnerDto(PartnerDto partnerDto) {
		getBankDto().setPartnerDto(partnerDto);
	}
}
