...
public class PanelLieferantAnsprechpartner extends PanelAnsprechpartner {
	...
	protected void setDefaults(Integer iSort) throws Throwable {
		...
		LieferantDto lieferantDto = DelegateFactory.getInstance().getLieferantDelegate().lieferantFindByPrimaryKey(iId);
		getInternalFrameLieferant().getAnsprechpartnerDto().setPartnerIId(lieferantDto.getPartnerIId());

		if (iSort == null) {
			// eine neue Position bekommt ein eindeutiges iSort
			iSort = DelegateFactory.getInstance().getAnsprechpartnerDelegate()
					.getMaxISort(getInternalFrameLieferant().getLieferantDto().getPartnerIId());
