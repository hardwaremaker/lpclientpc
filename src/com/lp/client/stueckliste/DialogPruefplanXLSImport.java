...
public class DialogPruefplanXLSImport extends JDialog implements
		ActionListener {
	/**
	 * 
	 */
	...
	public DialogPruefplanXLSImport(TabbedPane tp) throws Throwable {
		super(LPMain.getInstance().getDesktop(), LPMain.getInstance()
				.getTextRespectUISPr("stkl.pruefplan.importieren"), true);
		...
		XlsFileOpener xlsOpener = FileOpenerFactory.stklPruefplanImportOpenXls(tp.getInternalFrame());
	...
	private void jbInit() throws Throwable {
		...
		wtaFehler.setText(DelegateFactory.getInstance()
				...
				.pruefeUndImportierePruefplanXLS(xlsDatei, false,wcbVorhandenePositionenLoeschen.isSelected()));
		...
		if (e.getSource().equals(wbuImportieren)) {
			try {
				DelegateFactory.getInstance().getStuecklisteDelegate()
						.pruefeUndImportierePruefplanXLS(xlsDatei, true, wcbVorhandenePositionenLoeschen.isSelected());
