...
class ActionFormatColorBackground extends StyledTextAction {
	/**
	 *
	 */
	...
	public ActionFormatColorBackground() {
		super("FormatColorBackground");
		sName = LpEditorMessages.getInstance().getString("Action.ColorBackground");
		sShortDescription = sName;
		// kAcceleratorKey = KeyStroke.getKeyStroke(KeyEvent.VK_N,
		// KeyEvent.CTRL_MASK);
		iMnemonicKey = LpEditorMessages.getInstance().getMnemonic("Action.ColorBackground");
		imgIcon = IconManagerSingleton.getIconManager().getIcon(LpEditorIconManager.ICON_STYLE_COLOR_BACKGROUNG);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (textComp instanceof JTextPane) {
			...
			colorOld = editorPane.getStyledDocument().getBackground(attributeSetOld);
