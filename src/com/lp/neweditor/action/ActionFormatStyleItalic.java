...
class ActionFormatStyleItalic extends StyledEditorKit.ItalicAction {
	...
	public ActionFormatStyleItalic() {
		super();
		sName = LpEditorMessages.getInstance().getString("Action.Italic");
		sShortDescription = sName;
		kAcceleratorKey = KeyStroke.getKeyStroke(KeyEvent.VK_K, KeyEvent.CTRL_MASK | KeyEvent.SHIFT_MASK);
		iMnemonicKey = LpEditorMessages.getInstance().getMnemonic("Action.Italic");
		imgIcon = IconManagerSingleton.getIconManager().getIcon(LpEditorIconManager.ICON_STYLE_ITALIC);
